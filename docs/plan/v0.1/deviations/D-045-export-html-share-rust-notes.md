...
- T14 任务文件 W5：模板结构对齐上游；gist share 走
  `gh gist create --public=false` + `PIR_SHARE_VIEWER_URL` 拼接；gh 进程调用可注入。

## 实际实现与偏离原因

1. **模板资产编译期内嵌**：`template.html/.css/.js` 与
   `vendor/{marked,highlight}.min.js` 从上游逐字节复制到
   `crates/pir/src/core/export_html/` 并 `include_str!` 内嵌（单文件二进制，
   ADR-0002 §8），替代上游运行时 `getExportTemplateDir`（config.ts:412-419）
   读包目录；无 `PIR_PACKAGE_DIR` 式模板覆盖。字节一致性由
...
2. **`ToolHtmlRenderer` / `renderedTools` 未移植**：上游用扩展 TUI 渲染器经
   ...
   pir 无 JS 扩展渲染器，`renderedTools` 永不出现，viewer
...
6. **`/share` 并发模型**：gh 调用集中在 `core/share.rs` 的 `ShareRunner`
   ...
   `restoreEditor()` 口径一致。
7. **`PIR_SHARE_VIEWER_URL`**：`PI_` → `PIR_` 前缀改名（ADR-0001），读取集中在
