...
`crates/rpi-ai/src/api/azure_openai_responses.rs` 完整移植上游 `azure-openai-responses.ts`（`azure-openai-responses.lazy.ts` 为动态 import 包装，rpi 静态链接无对应物）。行为锚点（deployment name map、3 个 Azure host 后缀路径归一化到 `/openai/v1`、API version 默认 `v1`、base URL 四级回退链、strict tools 默认支持、reasoning effort/summary、encrypted_content 重放）均逐条对齐，消息/工具转换与流处理复用 T03 的 `openai_responses_shared.rs`。落地差异：
...
2. **`on_payload` 见 wire（snake_case）JSON**，而非 SDK 的 camelCase `ResponseCreateParamsStreaming` 对象（与 rpi 其他适配器一致）。
...
5. **`stream_simple` 缺 API key 报错进事件流**：上游 `streamSimple` 同步 throw；rpi 与其他适配器一致地走 `immediate_error_stream`（D-004 形状）。
6. **scratch 字段无需擦除**：上游 catch 块删除 content block 上的 `partialJson`/`customInput` 暂存字段；rpi 的流式暂存留在 `ResponsesStreamProcessor` 内部，从不进入 content block。

## 影响面

无（纯内部）：请求线格式（路径/query/头/体形状）经钉死版 SDK 源码核对一致；base URL 归一化、deployment map、API version 默认、reasoning replay、tool-call、错误流均有契约测试对拍（`crates/rpi-ai/tests/contract_azure_openai_responses.rs`）。偏差点集中在 SDK 自有头与解析/错误文案，不进 session/RPC 线格式。
