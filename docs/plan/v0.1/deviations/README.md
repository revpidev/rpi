...
| D-044 | T14 | 行为级 | 交互模式 switchSession 异 cwd 信任提示降级为 headless 判定（无 TUI 弹窗，ask→false + untrusted warning，`/trust` + 重启生效；同 cwd 缓存路径与上游一致；T15 异步选择器桥接就位后接线关闭） | T14 偏离表、D-043 遗留项 | ADR-0006 | 已回写 |
