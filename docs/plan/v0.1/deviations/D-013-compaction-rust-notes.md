...
- 原文约定：
  - §12 映射表：`packages/coding-agent/src/core/compaction/*` → `crates/rpi/src/core/compaction/*`；
...
1. **算法层落点为 `rpi-agent::compaction`，而非 `rpi::core::compaction`**。
   设计 §4.5 已规定 compaction/branch-summary 的算法与常量「抽到 `rpi-agent` 公共模块
   供 coding-agent 与 harness（T16）两处复用」，§12 映射表未同步这一层。落地拆分：
   - `crates/rpi-agent/src/compaction.rs`（+ `compaction/utils.rs`、`compaction/branch_summarization.rs`）：
     `compaction.ts` / `utils.ts` / `branch-summarization.ts` 的逐字节移植（估算、切点、
     prompt 模板、summary 生成、branch 装填），coding-agent 与 T16 harness 共用；
   - `crates/rpi/src/core/compaction_runner.rs`：仅 coding-agent 侧触发接线
     （`_checkCompaction` 双路、overflow 一次恢复、`_runAutoCompaction`、事件发射），
     即原映射表 §12 对应物的实际位置。
2. **`rpi-ai::types::StreamOptions` 增加 `reasoning: Option<ModelThinkingLevel>` 字段**。
   上游 summary 调用走 `complete_simple` 经 `SimpleStreamOptions.reasoning` 传 thinking
   level（`createSummarizationOptions`，compaction.ts:539-553）；rpi-agent 的 summary
...
3. **session 条目→context 消息的共享函数下沉**：
   ...
   / `build_context_messages` 落在 `rpi-agent::session`，`rpi::core::session_manager`
