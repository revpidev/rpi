...
- 文档与章节：`docs/02-design.md` §3.2（pir-ai 核心类型）、§4.1（pir-agent 分层表）；`docs/coding-standards.md` §2.3（公开类型命名保留上游拼写）
- 原文约定：核心类型「镜像 Pi」；`pir-agent` 的 `types` 模块含「AgentEvent / AgentTool / AgentMessage（含扩展消息文本格式常量）」。
...
1. **声明合并折叠**：coding-agent 用 `declare module` 把 `bashExecution` / `custom` /
   ...
   合并，四个自定义消息类型直接折叠进 `pir-agent::messages::AgentMessage` 联合
   （`messages.rs` 溯源注释已注明）。
...
无（纯内部）。线格式形状由 T01 快照测试锁定（`pir-ai/src/types.rs`、
`pir-agent/src/{types,messages}.rs` tests，30 例）。
