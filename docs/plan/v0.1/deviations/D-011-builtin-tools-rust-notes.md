...
2. **图像处理栈**：上游用 Photon（Rust/WASM）+ 手写 EXIF 解析器；rpi 用 `image` crate（纯 Rust，default-features 关闭仅启用 png/jpeg/gif/webp/bmp）做解码/缩放/编码（Lanczos3、`[80,85,70,55,40]` 质量梯度、×0.75 尺寸回退与上游一致），`kamadak-exif` 做 EXIF 方向校正（JPEG/WebP 等价；PNG/GIF/BMP 无 EXIF 两侧均为无操作）。GIF 缩放取首帧，与上游 Photon 单帧行为一致。图像字节级输出不保证与上游一致（编码器不同），但 content block 形状、mime 类型、hint 文案为对拍契约面，均字面对齐。
3. **diff 生成自实现 Myers**：上游用 jsdiff 的 `diffLines`/`createTwoFilesPatch`；rpi 在 `edit_diff.rs` 自实现 O(ND) Myers 行级 diff，并在其上生成自定义 diff（`+N/-N/ N` 前缀、上下文折叠四分支）与 unified patch（`---/+++` 头、context 4、`\ No newline at end of file`）。`details.diff/patch` 不进 Agent 层对拍契约（fixture 剥离 details），文本形状有移植测试锚定。
...
6. **`trackDetachedChildPid` 全局注册表未移植**：上游用于父进程崩溃时清理孤儿 detached 进程组；rpi 取消/超时路径的进程组 SIGKILL 语义完整（有「取消后无残留」测试锚定），崩溃兜底为已知简化，后续需要时补。
...
9. **`getShellEnv` 前置 `~/.rpi/bin`**：上游前置 `getBinDir()`（agentDir/bin = `~/.pi/bin`）；rpi 配置根为 `~/.rpi`（ADR-0001），等价前置 `~/.rpi/bin`。RPI_* 环境变量前缀重命名同为 ADR-0001 钉死项，非偏离。
10. **bash 节流更新实现**：上游 `Date.now()` 节流 + 定时器合并；rpi 用 unbounded mpsc + drain 合并（收到通知先 drain 队列、距上次不足 100ms 则睡眠后再 drain），首次数据立即更新；合并语义等价（5000 行 chatty 输出更新数 < 25 的移植测试锚定）。
...
无（纯内部）。对拍契约面（事件序、content 文本、错误文案、截断/环境注入/节流行为）逐项有移植测试或 fixture 归一化 diff 锚定：`rpi/tests/parity_tools_test.rs`（真实 read+bash 对 `tool-calls` fixture）。
