...
核心落 `crates/rpi/src/core/package_manager.rs`（update 编排 + `resolve_all`）、`core/version_check.rs`、`config.rs`（self-update 段）、`cli/package_command.rs`（update 分支）。契约要点全部就位；实现层差异：

1. **并发模型**：上游 `runWithConcurrency` 是 promise worker pool；rpi 的包管理方法是同步的，用 `std::thread::scope` worker pool 实现（结果保序，上限 4 各自独立）。多重失败时返回**任务序第一个**错误（上游是时间上第一个 reject）；单失败语义一致。
...
3. **自更新机制按上游忠实移植**（npm/pnpm/yarn/bun 全局重装 + 改名先卸载），**不含 GitHub release 二进制下载**——上游对编译二进制（bun-binary）同样只打印 releases 页提示。rpi 恒为原生二进制：`detectInstallMethod` 只查当前可执行文件路径（上游还拼 `__dirname`；rpi 无对应物），无法区分「bun-binary」与「unknown」两种上游结局，统一按 bun-binary 处理（打印 `Download from: {SELF_UPDATE_DOWNLOAD_URL}`）。
4. **`PACKAGE_NAME = "rpi"` 占位常量**：rpi 尚无已发布的 npm 包；release endpoint 的 `packageName` 改名重定向机制完整保留。endpoint 集中在 `core/version_check.rs::LATEST_VERSION_URL` 与 `config.rs::SELF_UPDATE_DOWNLOAD_URL` 两个常量，W6 的 `RPI_*` endpoint 配置化在这两处接口子。
5. **版本检查 HTTP 可注入**：`LatestVersionTransport` trait（生产为 reqwest+rustls；测试为脚本化 stub）；`RPI_OFFLINE` 语义复用 package-manager 的 truthy 判定（上游是「非空即真」，rpi 与全仓 RPI_OFFLINE 口径一致为 `1/true/yes`，同 D-040 的口径选择）。UA 为 `rpi/{version} ({os}; rust; {arch})`（D-038 先例）。
6. **release note 渲染**走 rpi-tui Markdown 组件 + identity 主题（headless 纯文本约定，同 D-040 #13）；宽度取 stdout ioctl TIOCGWINSZ，非 TTY 回退 80（上游 `process.stdout.columns ?? 80`）。
...
11. **update 路径信任**：严格 `useSavedProjectTrustOnly`——无资源扫描、无 UI、无扩展事件；`-a/-na` 覆盖；trust.json 读取失败 → `Error: …` exit 1（上游会向上抛，main 层兜底为未处理拒绝）。「`rpi update` 永不提示」约束有测试锚点（`update_extensions_skips_untrusted_project_packages`）。
...
2. **usage 文案与 `APP_NAME` 绑定**：`UPDATE_USAGE`/`INSTALL_USAGE`/`REMOVE_USAGE`/
   `LIST_USAGE` 中的字面 `rpi` 与 `config::APP_NAME` 之间新增
