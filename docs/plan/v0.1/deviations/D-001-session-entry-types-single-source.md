# D-001：session 条目类型单一来源化（合并至 `rpi-agent::session`）
...
- 原文约定：`packages/coding-agent/src/core/session-manager.ts` → `crates/rpi/src/core/session_manager.rs`；`packages/agent/src/harness/*` → `crates/rpi-agent/src/harness/*`；「一个上游文件对应一个同名 Rust 文件」。
...
实际实现：两套定义合并为单一来源 `crates/rpi-agent/src/session.rs`——每种条目一个
...
`rpi` 主路径（T07）与 harness（T16）共用此模块；「主路径只写 `firstKeptEntryId`」
的钉死版行为由写入方纪律保证（ADR-0003 §1 的措辞不变）。

选择放在 `rpi-agent` 而非 `rpi`：harness（rpi-agent 内）需要这些类型，依赖方向
`rpi → rpi-agent` 单向不可逆（coding-standards §2.2）。

## 影响面

无（纯内部）。serde 线格式与上游逐字段一致（含 `parentId`/`targetId` 显式 null、
`| undefined` 字段缺省省略），有快照测试覆盖（`rpi-agent/src/session.rs` tests）。
