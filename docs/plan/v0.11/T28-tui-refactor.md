...
pir-tui 从单一 Tui struct 重构为「trait + 基类复用 + 双渲染器」结构，main-screen 渲染逻辑
...
- 重构后 pir 主路径（interactive-mode 等）全部调用点编译迁移，行为不变
- 上游 main-screen 等价验证（旧 `TUI.doRender` 368 行 vs 新 368 行仅两处等价改写）作为 pir 侧迁移的核对基准
