# Rpi Parity Checklist（T15 W8 Parity Freeze）
...
> （live 测试未设 `RPI_LIVE_TEST=1` 默认跳过）。
...
| 适配器 | 实现锚点 | 对拍证据 |
|--------|----------|----------|
| anthropic-messages | `crates/rpi-ai/src/api/anthropic_messages.rs` | `crates/rpi-ai/tests/contract_adapters.rs`、`providers_group_a.rs` |
| openai-completions | `crates/rpi-ai/src/api/openai_completions.rs` | `contract_adapters.rs`、`providers_group_*.rs` |
| openai-responses | `crates/rpi-ai/src/api/openai_responses.rs` | `contract_adapters.rs` |
| pi-messages | `crates/rpi-ai/src/api/pi_messages.rs` | `contract_pi_messages.rs`（T13，D-021） |
| mistral-conversations | `crates/rpi-ai/src/api/mistral_conversations.rs` | `contract_mistral_conversations.rs`（D-022） |
| google-generative-ai | `crates/rpi-ai/src/api/google_generative_ai.rs` | `contract_google_generative_ai.rs`（D-023） |
| azure-openai-responses | `crates/rpi-ai/src/api/azure_openai_responses.rs` | `contract_azure_openai_responses.rs`（D-024） |
| google-vertex（ADC 子集） | `crates/rpi-ai/src/api/google_vertex.rs` | `contract_google_vertex.rs`（D-025） |
| bedrock-converse-stream（手写 SigV4 + event-stream） | `crates/rpi-ai/src/api/bedrock_converse_stream.rs` | `contract_bedrock_converse_stream.rs`（D-026） |
| openai-codex-responses（WS 状态机 + 缓存续传 + zstd） | `crates/rpi-ai/src/api/codex_ws.rs` | `contract_openai_codex_responses.rs`（D-027；状态机图钉死于 `02-design.md` §13） |
| compat 修正矩阵 | `crates/rpi-ai/src/api/openai_completions.rs:199`（`detect_compat` 表驱动） | `crates/rpi-ai/tests/compat_matrix.rs` |
| 38 provider 工厂 + 模型目录（37 vendored JSON / 1153 模型） | `crates/rpi-ai/src/providers/` | `providers_group_a-d.rs`、`model_catalog.rs` |
| OAuth 6 流程 + load registry | `crates/rpi-ai/src/auth/` | `oauth_codex_openrouter.rs`、`oauth_copilot_radius.rs`、`oauth_kimi_xai.rs`（D-029~D-035） |
| images 子系统 | `crates/rpi-ai/src/images.rs`、`images/` | `crates/rpi-ai/tests/images.rs`（D-037） |

事件流线格式对拍：`fixtures/generated/<scenario>/events.jsonl`（上游
`createAgentSession` + faux 实录，`fixtures/generate-fixtures.mjs` 生成）vs
rpi 全栈事件流 —— `crates/rpi/tests/parity_headless_test.rs`（5 场景，
归一化口径见文件头注释）。

### 1.2 RPC 帧协议（32 命令）

- 命令枚举：`crates/rpi/src/modes/rpc.rs:98`（`RpcCommand`，32 变体：
...
- 测试锚点：`crates/rpi/tests/rpc_mode_test.rs`（36 用例）——
...
- 9 协议方法 = 4 对话（`select`/`confirm`/`input`/`editor`，id 配对阻塞等待
  ...
  `crates/rpi/src/modes/rpc.rs:49-57`。
- 18 降级方法逐项语义：`EXTENSION_UI_DEGRADED_METHODS`（rpc.rs:68 起，
  注释逐条对齐 rpc-mode.ts:162-309）。
- 实现锚点：`crates/rpi/src/modes/rpc/ui_bridge.rs:127`（`RpcUiBridge`）。
- 测试锚点（W4 契约）：`w4_rpc_select_frame_shape_and_value_response`、
  ...
  `w4_rpc_degraded_methods`（`crates/rpi/tests/extension_host_w4_test.rs`）。
...
| 项 | 证据 |
|----|------|
| JSONL v3 无损往返（加载 → export 归一化 diff） | `parity_fixture_sessions_load_and_export_lossless`（`crates/rpi/tests/parity_session_test.rs`；5 个上游实录 fixture：`abort`/`length-truncation`/`single-turn`/`steering-followup`/`tool-calls`） |
...
| v1–v3 自动迁移（主路径） | `crates/rpi/src/core/session_manager.rs` 迁移单测（`session_manager::tests`）+ `parity_session_test.rs` 头注口径 |
| compaction 对拍（threshold/overflow 两场景 golden） | `crates/rpi/tests/parity_compaction_test.rs`；golden 由 `fixtures/generate-compaction-golden.mjs` 生成 |
| harness v3 硬校验 + 双向互通（harness↔主路径） | `crates/rpi/tests/parity_harness_interop_test.rs`（T16：11 种条目全量、两形态 compaction、fixtures 三方交叉） |
| 基准文档 | `external/pi/packages/coding-agent/docs/session-format.md`（G3 逐条对拍级基准） |
| rpi 产出被上游格式校验 | 上游无独立校验器；口径 = rpi 产出 JSONL 与上游 fixture 经同一 Normalizer（`rpi-test-support/src/normalize.rs`）归一化 diff（`parity_headless_test.rs` session 对拍分支），即「rpi 产出 ≡ 上游实录」的反向表达 |

---

## 3. 扩展 API（33 事件 + 24 API + 28 UI + 3 Context = 88 条）

宿主实现：`crates/rpi-ext-host/`；会话侧接缝：`crates/rpi/src/core/extensions.rs`
...
| # | 事件 | 触发锚点（rpi） | 分发锚点（rpi-ext-host） | 测试锚点 |
|---|------|----------------|--------------------------|----------|
...
pi 对象实现：`crates/rpi-ext-host/src/api.rs`（`ExtensionApi`）；动作绑定：
`crates/rpi/src/core/extension_actions.rs`（`SessionHostActions`）。
...
trait：`crates/rpi-ext-host/src/api.rs:412-516`（`UiBridge`）。三桥：
...
`crates/rpi/src/modes/interactive/component_tree.rs`；测试
...
- ABI v1：`docs/extension-abi.md`；宿主 `crates/rpi-ext-host/src/wasm/`。
...
| 项 | 证据 |
|----|------|
| 组件渲染快照黄金（36 个，ANSI 逐字节） | `crates/rpi-tui/tests/snapshots.rs` + `tests/snapshots/*.snap`（36 文件） |
| 差分渲染/帧泵/终端控制单测 | `crates/rpi-tui/src/tui.rs`（123 例）、`terminal.rs`（18 例）内联测试 |
| VT 帧工具 | `rpi-test-support/src/vt.rs`（帧捕获/消毒/纯文本投影） |
| 键位映射 | `crates/rpi/tests/keybindings_test.rs`（26 例）+ `docs/plan/v0.1/T12-keybindings-mapping.md`（逐条映射表） |
...
| 差异 | ADR | 偏离登记 |
|------|-----|----------|
| CLI 名 `rpi`、配置根 `~/.rpi`、env 前缀 `RPI_*`、扩展 = Rust/Wasm（不兼容 TS/jiti） | ADR-0001 / ADR-0002 | —（第 3 层有意差异，需求 §1.5） |
...
| 3 | 资源互通（skills/prompts/themes/settings/keybindings/models.json + 凭据存储结构兼容，仅路径 ~/.rpi） | ✅ 通过 | `parity_resources_test.rs`、`skills_test.rs`、`prompt_templates_test.rs`、`themes_test.rs`、`keybindings_test.rs`、`resource_loader_test.rs`；golden 生成器 `fixtures/generate-resources-golden.mjs`；凭据存储 `rpi-ai/src/auth/`（D-008 结构兼容、路径 `~/.rpi`） |
| 4 | 架构同构（四层 crate ↔ Pi 四包） | ✅ 通过 | `rpi-ai`↔pi-ai、`rpi-agent`↔pi-agent（harness）、`rpi-tui`↔pi-tui、`rpi`↔coding-agent；`02-design.md` §12 映射表 |
