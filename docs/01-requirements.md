...
- **Markdown**：marked 等价解析；`trimPartialClosingFences()` 流式 fence 稳定（防代码块闪烁）；code block border + indent；主题 20+ 样式函数；语法高亮
