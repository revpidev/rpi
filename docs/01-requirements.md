...
- `/new`、import/export JSONL、HTML export、gist share（shell 调 `gh gist create --public=false` + `RPI_SHARE_VIEWER_URL` 拼接，默认 `https://revpi.dev/session/` 可配，ADR-0009）
...
- 版本检查 / telemetry：**支持配置自有 endpoint**（settings / `RPI_*`，默认 `revpi.dev` 可改，ADR-0009）；可关闭；`enableInstallTelemetry`/`enableAnalytics`（opt-in）
