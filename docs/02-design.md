...
**产品 endpoint 配置化**（ADR-0002 §8，T14-W6a，D-046）：版本检查 / install telemetry / 远程模型目录三类产品 HTTP 回调的 endpoint 在 settings 与 `RPI_*` env 可配置、可整体关闭。统一解析器 `config::resolve_endpoint`：env（`RPI_VERSION_CHECK_URL` / `RPI_TELEMETRY_URL` / `RPI_MODEL_CATALOG_URL`）> settings（`versionCheckUrl` / `telemetryUrl` / `modelCatalogUrl`，camelCase）> 内置默认（`https://revpi.dev`，与上游硬编码值 `https://pi.dev` 不同——ADR-0009 独立分发决策）；任一级取字面量 `off` 即关闭，关闭后不产生任何网络请求。消费点：`update --self/--all` 的版本探测、交互模式启动版本检查通知（`UiCommand::NewVersionAvailable`）、install telemetry ping（`core/telemetry.rs`，默认开、`RPI_TELEMETRY`/`enableInstallTelemetry` 可关、payload 仅版本号）；远程 catalog 的运行时消费随内置 provider 注册波次接线（D-038，2026-08-09 D-052 补齐：`ModelRuntime::create` 播种 38 内置 provider，静态目录类逐个包 `with_remote_catalog`（radius 原样），settings `modelCatalogUrl` 经 services 层传入，SDK/`update --models` 走 env/默认；字面量 `off` 不构造 overlay、零请求）。`enableAnalytics` 默认 false 且上游 0.82.1 无发送通道，按构造零请求。
