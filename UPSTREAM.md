...
升级说明：v0.11 将对照基线从 `2efa728`（v0.82.1）提升至 `4181f66`（v0.84.1+），跨度 461 commits / 655 文件。变更需求与设计见独立文档仓库（未公开）。

历史基线：v0.1 钉死 `2efa728d2ee90ef597626e96b1e28ef2b279f07c`（v0.82.1，2026-07-27）。

已立 ADR 的有意差异（不在金标准范围内）：产品端点默认值迁移 `revpi.dev`（含 rpi-pages 仓库的 Cloudflare Pages 部署）；覆盖链与上游端点可配置语义不变。
