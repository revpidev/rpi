...
# ADR-0025 §9; re-pinned to v2.34.0+9 by TE37 under ADR-0029 for the
# v0.1.5 rebase window). Read-only against external/: `git archive` reads the
# object database only (no checkout, no worktree), so
# `git -C external/pi-mcp-adapter status --porcelain` stays empty and the
# submodule HEAD is untouched. The submodule HEAD remains the OLD pin
# (v2.32.1) until TE40 performs the atomic switch, so this snapshot is the
# pre-switch stand-in for the new target pin.
#
...
#
#   RPI_MCP_PARITY_UPSTREAM=/tmp/rpi-mcp-parity-target-v2340
#   RPI_MCP_PARITY_DEPS=/tmp/rpi-mcp-parity-target-v2340
#
# Usage:
#   bash scripts/mcp-parity/setup-target-source.sh
#
# Environment:
#   RPI_MCP_TARGET_SRC  snapshot/deps dir (default /tmp/rpi-mcp-parity-target-v2340)
#   RPI_MCP_TARGET_PIN  target commit (default 97435aab..., v2.34.0+9)
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
PIN="${RPI_MCP_TARGET_PIN:-97435aabf74e5fbcf1112e7244f931172b9db624}"
DEST="${RPI_MCP_TARGET_SRC:-/tmp/rpi-mcp-parity-target-v2340}"
