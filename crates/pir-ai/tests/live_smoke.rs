...
use pir_ai::api::azure_openai_responses::AzureOpenAiResponses;
use pir_ai::api::bedrock_converse_stream::BedrockConverseStream;
use pir_ai::api::google_generative_ai::GoogleGenerativeAi;
use pir_ai::api::mistral_conversations::MistralConversations;
...
const MISTRAL: LiveTarget = LiveTarget {
    ...
    model_env: "PIR_LIVE_MISTRAL_MODEL",
    base_url_env: "PIR_LIVE_MISTRAL_BASE_URL",
...
const GOOGLE_GENERATIVE_AI: LiveTarget = LiveTarget {
    ...
    model_env: "PIR_LIVE_GOOGLE_MODEL",
    base_url_env: "PIR_LIVE_GOOGLE_BASE_URL",
...
fn gate_azure() -> Option<(Model, StreamOptions)> {
    ...
    let base_url = std::env::var("PIR_LIVE_AZURE_BASE_URL")
        .ok()
        .unwrap_or_else(|| {
    ...
    let model_id =
        std::env::var("PIR_LIVE_AZURE_MODEL").unwrap_or_else(|_| "gpt-4o-mini".to_owned());
...
/// then `us-east-1` and can be overridden via `PIR_LIVE_BEDROCK_BASE_URL`.
fn gate_bedrock() -> Option<(Model, StreamOptions)> {
    ...
    let base_url = std::env::var("PIR_LIVE_BEDROCK_BASE_URL")
        .ok()
        .unwrap_or_else(|| {
    ...
    let model_id = std::env::var("PIR_LIVE_BEDROCK_MODEL")
