...
//! (`crates/pir-ai/src/api/openai_codex_responses.rs`), porting the intent of
...
use pir_ai::api::openai_codex_responses::{
...
async fn run_aged(m: &Model, session: &str) -> Option<pir_ai::types::AssistantMessage> {
    codex_stream(
        m,
        &context(vec![user_text("hi")]),
        OpenAiCodexResponsesOptions {
