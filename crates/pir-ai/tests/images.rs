...
use pir_ai::auth::AuthContext;
use pir_ai::images::{builtin_images_models, generate_images, get_image_model};
use pir_ai::types::{
...
async fn image_input_is_sent_as_data_url() {
    ...
    let context = ImagesContext {
        input: vec![
            ...
            ImagesInputContent::Image(pir_ai::types::ImageContent {
...
async fn on_payload_replaces_payload_and_on_response_sees_status() {
    let (base_url, mut requests) =
        serve(vec![ScriptResponse::json(200, image_response_body())]).await;
    let model = test_model(&base_url, vec![ImagesOutputModality::Image]);
    let on_payload: pir_ai::types::ImagesOnPayloadCallback = Arc::new(
        |payload: Value,
         _model: &ImagesModel|
         -> std::pin::Pin<
    ...
    let on_response: pir_ai::types::ImagesOnResponseCallback =
...
async fn builtin_images_models_registers_openrouter_with_catalog() {
    let models = builtin_images_models(Some(pir_ai::models::CreateModelsOptions {
        credentials: None,
