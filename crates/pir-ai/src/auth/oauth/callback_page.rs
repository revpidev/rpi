...
//! read from `PIR_OAUTH_CALLBACK_HOST` (ADR-0001 §2 `PIR_` prefix; upstream
...
/// Env override for the bind host (ADR-0001 §2 `PIR_` prefix; corresponds to
/// upstream `PI_OAUTH_CALLBACK_HOST`).
const CALLBACK_HOST_ENV: &str = "PIR_OAUTH_CALLBACK_HOST";

/// Upstream: `getProviderEnvValue("PI_OAUTH_CALLBACK_HOST") || "127.0.0.1"`
/// (here with the `PIR_`-prefixed variable). Shared with the anthropic flow,
...
impl OAuthCallbackServer {
    /// Bind `CALLBACK_PORT` on `PIR_OAUTH_CALLBACK_HOST` (default 127.0.0.1).
    pub async fn start(
