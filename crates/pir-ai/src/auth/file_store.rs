...
//!   path resolution (`~/.pir/agent/auth.json`) belongs to the unified path
//!   module (T09).
...
/// newer pir versions do not model — round-trip verbatim, and serialized
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        ...
        let dir =
            std::env::temp_dir().join(format!("pir-test-auth-storage-{}-{id}", std::process::id()));
    ...
    async fn dsl_fixture_keys_resolve_on_read() {
        let _guard = EnvGuard::set("PIR_FIXTURE_API_KEY", "fixture-env-key");
        let dir = temp_dir();
        let auth_path = dir.join("auth.json");
