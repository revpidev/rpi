...
//! pir adapters are linked statically, so there is no lazy counterpart.)
//!
...
//!   Bedrock — pir honors an explicit value).
...
//!   to the SDK profile chain) pir falls back to `us-east-1`, matching
...
///
/// `profile` is accepted for upstream parity but inert: pir's credential
...
/// (ambient profile); pir then falls back to `us-east-1`
...
/// defers to the ambient profile (SDK chain upstream, `us-east-1` in pir).
...
/// live in a side map (pir's content blocks carry no `index` field), the
...
async fn run(
    ...
    let response = retry_provider_request(
        ...
        ProviderRetryOptions {
            ...
            // attempts) and ignores `maxRetries` for Bedrock; pir defaults
