...
//!   the `SimpleStreamOptions` object) is not ported: pir's
...
//!   `PIR_CACHE_RETENTION` (requirements §5.5); unlike the anthropic adapter,
...
fn resolve_cache_retention(
    ...
    if get_provider_env_value("PIR_CACHE_RETENTION", env).as_deref() == Some("long") {
    ...
    fn convert(&mut self, event: PiMessagesEvent) -> StreamEvent {
        match event {
            ...
            PiMessagesEvent::TextDelta {
                ...
                // Upstream's `as` cast would throw on a non-text block; pir
...
/// off the options object; pir's [`SimpleStreamOptions`] has no such fields,
...
mod tests {
    ...
    fn test_resolve_cache_retention() {
        ...
        let env: ProviderEnv = [("PIR_CACHE_RETENTION".to_owned(), "long".to_owned())].into();
        assert_eq!(
            resolve_cache_retention(None, Some(&env)),
            Some(CacheRetention::Long)
        );
        let env: ProviderEnv = [("PIR_CACHE_RETENTION".to_owned(), "short".to_owned())].into();
