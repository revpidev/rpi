...
//!   sent, and the SDK performs no retries by default, so
//!   `StreamOptions::max_retries` is a no-op for this adapter.
...
fn finish_processor(
    ...
    match processor.output.stop_reason {
        StopReason::Pending => Err("Google stream ended without a finish reason".to_owned()),
