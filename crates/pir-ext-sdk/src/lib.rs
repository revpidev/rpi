//! `pir-ext-sdk` — guest-side SDK for pir wasm extensions (ABI v1).
//!
...
//! through [`export!`]. The host calls `pir_extension_init` once (your
//! registrations run there) and `pir_dispatch` per event/tool call; guest
...
macro_rules! export {
    ($register:path) => {
        ...
        #[no_mangle]
        pub extern "C" fn pir_dispatch(ptr: u32, len: usize) -> u64 {
...
/// `pir_extension_init` tail: push registrations to the host, then return
...
/// `pir_dispatch` entry: route the message to the registered handler.
#[doc(hidden)]
