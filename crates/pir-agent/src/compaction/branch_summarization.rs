...
//! `pir::core`, owns the session store). Semantics are identical: the
//! parent-id walk from the old leaf visits exactly the `old_path` suffix
//! after the common ancestor.

use pir_ai::types::{Model, StopReason, StreamOptions, Usage};
...
pub async fn generate_branch_summary(
    ...
    let summarization_messages = vec![pir_ai::types::Message::User(pir_ai::types::UserMessage {
    ...
    let context = pir_ai::types::Context {
