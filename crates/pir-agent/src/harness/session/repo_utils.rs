...
//!   `pir` crate, so the same algorithm is re-implemented here.
...
// same algorithm as session_manager.rs:62-105, private to the `pir` crate)
// ---------------------------------------------------------------------------
...
pub(crate) mod test_support {
    ...
    impl TestFs {
        fn new_inner(deny_read_text_file: bool) -> Arc<Self> {
            let root =
                std::env::temp_dir().join(format!("pir-agent-session-test-{}", unique_suffix()));
