...
use pir_agent::harness::session::memory_storage::{
...
use pir_agent::messages::{AgentMessage, CustomMessage, CustomRole};
use pir_agent::session::{MessageEntry, SessionEntry};
use pir_agent::types::{AgentEvent, AgentToolResult, AgentToolUpdateCallback, QueueMode};
...
fn text_from_user_messages(context: &Context) -> Vec<String> {
    context
        ...
        .filter_map(|message| match message {
            pir_ai::types::Message::User(user) => Some(user_message_text(user)),
            _ => None,
        })
        .collect()
}
...
where
    ...
    Fut: std::future::Future<
            Output = Result<pir_agent::harness::HarnessHookResult, AgentHarnessError>,
        > + Send
        + 'static,
{
...
async fn test_construct_exposes_queue_modes_body() {
    ...
    let harness = AgentHarness::new(AgentHarnessOptions {
        ...
        system_prompt: Some(pir_agent::harness::AgentHarnessSystemPrompt::Static(
            ...
            .map(|text| {
                ...
                FauxResponseStep::Factory(Box::new(move |context, _options, _state, _model| {
                    let count = context
                        .messages
                        .iter()
                        .filter(|m| matches!(m, pir_ai::types::Message::User(_)))
                        .count();
...
async fn test_before_agent_start_appends_and_persists_messages_body() {
    ...
    let _unsubscribe = harness.on(
        "before_agent_start",
        hook(|_event| async {
            Ok(pir_agent::harness::HarnessHookResult::BeforeAgentStart(
    ...
    let harness = Arc::new(
        AgentHarness::new(AgentHarnessOptions {
            ...
            resources: AgentHarnessResources {
                skills: Some(vec![pir_agent::harness::Skill {
            ...
            system_prompt: Some(pir_agent::harness::AgentHarnessSystemPrompt::Dynamic(
                Arc::new(|context| {
                    let prompt = context
                        .resources
                        .skills
    ...
    let _unsubscribe = harness.on("tool_call", {
        ...
        hook(move |event| {
            ...
            async move {
                ...
                Ok(pir_agent::harness::HarnessHookResult::ToolCall(None))
            }
        })
    });
    let _unsubscribe2 = harness.on("tool_result", {
        ...
        hook(move |event| {
            ...
            async move {
                ...
                Ok(pir_agent::harness::HarnessHookResult::ToolResult(Some(
                    ToolResultPatch {
        ...
        hook(move |event| {
            ...
            async move {
                let AgentHarnessOwnEvent::SessionBeforeCompact { preparation, .. } = event else {
                    return Ok(pir_agent::harness::HarnessHookResult::SessionBeforeCompact(
                        None,
                    ));
                };
                Ok(pir_agent::harness::HarnessHookResult::SessionBeforeCompact(
                    Some(pir_agent::harness::types::SessionBeforeCompactResult {
                        cancel: None,
...
fn operation_label(operation: pir_agent::harness::RetryOperation) -> &'static str {
    match operation {
        pir_agent::harness::RetryOperation::Compaction => "compaction",
        pir_agent::harness::RetryOperation::BranchSummary => "branch_summary",
    }
}
...
async fn test_branch_summary_retries_transient_errors_and_emits_events_body() {
    ...
    let result = harness
        .navigate_tree(
            &target_id,
            pir_agent::harness::NavigateTreeOptions {
                summarize: true,
                ..Default::default()
            },
        )
...
async fn test_branch_summary_persists_hook_provided_usage_body() {
    ...
    let _unsubscribe = harness.on("session_before_tree", {
        ...
        hook(move |_event| {
            let usage = usage.clone();
            async move {
                Ok(pir_agent::harness::HarnessHookResult::SessionBeforeTree(
                    Some(pir_agent::harness::types::SessionBeforeTreeResult {
                        summary: Some(pir_agent::harness::TreeSummary {
...
async fn test_resources_getters_and_update_events_body() {
    ...
    let skill = pir_agent::harness::Skill {
    ...
    let prompt_template = pir_agent::harness::PromptTemplate {
