...
use pir_agent::harness::utils::shell_output::{
...
async fn test_streams_stdout_and_stderr_chunks() {
    ...
    assert_eq!(
        result,
        pir_agent::harness::types::ShellExecResult {
...
async fn test_capture_reports_truncation_counts() {
    ...
    assert_eq!(
        ...
        Some(pir_agent::harness::utils::truncate::TruncatedBy::Lines)
    );
...
async fn test_capture_streams_chunks_with_progress() {
    ...
    let on_chunk = {
        ...
        Box::new(move |chunk: &str, _progress: &pir_agent::harness::utils::shell_output::ShellCaptureProgress| {
