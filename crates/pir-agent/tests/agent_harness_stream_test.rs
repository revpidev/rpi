...
use pir_agent::types::{AgentEvent, AgentToolResult, AgentToolUpdateCallback};
use pir_ai::auth::{
    ApiKeyAuth, ApiKeyCredential, AuthContext, AuthResult, ModelAuth, ModelsError, ProviderAuth,
};
use pir_ai::models::{Models, Provider};
use pir_ai::types::{
...
#[async_trait]
impl pir_agent::harness::AgentHarnessTool<()> for CalculateTool {
    fn name(&self) -> &str {
        "calculate"
    }
        ...
        FauxResponseStep::Factory({
            ...
            Box::new(move |_context, options, _state, _model| {
                ...
                faux_assistant_message(
                    ...
                    pir_test_support::faux::FauxAssistantOptions {
                        stop_reason: Some(pir_ai::types::StopReason::ToolUse),
                        ..Default::default()
                    },
                )
            })
