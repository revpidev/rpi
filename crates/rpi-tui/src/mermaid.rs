//! Port of the Mermaid Unicode box-drawing terminal renderer from
//! [xai-org/grok-build](https://github.com/xai-org/grok-build)
//! `crates/codegen/xai-grok-markdown/src/mermaid.rs` @
//! e5fd4816d43260c15ba785f103990c1ed6cea230 (Apache-2.0).
//!
//! Renders `graph`/`flowchart`, `sequenceDiagram`, `stateDiagram`,
//! `classDiagram`, and `erDiagram` blocks as Unicode box-drawing art;
//! unsupported diagram types fall back to the raw source in a framed box.
//!
//! Intentional differences from the upstream source:
//! - ratatui adapter: upstream builds `ratatui::text::Line`s; rpi-tui does not
//!   depend on ratatui (coding-standards §8.1) and components emit ANSI
//!   strings. The ratatui subset the blueprint consumes — `Style` (fg/bold/
//!   italic via `add_modifier`), `Span::styled`, `Line::from` — is reproduced
//!   by a local shim below (the `Span`/`Line` half stays private; `Style`,
//!   `Color`, `Modifier` are public so callers can build the five theme
//!   styles), and every shim `Line` is serialized to a
//!   crate-convention ANSI string (truecolor `38;2;…` / 256-color `38;5;…`
//!   foreground, `1`/`22` bold, `3`/`23` italic) at the `MermaidArt`
//!   construction sites via `to_ansi_lines`. `MermaidArt` therefore carries
//!   `styled_lines: Vec<String>` instead of upstream's `Vec<Line<'static>>`;
//!   `plain_lines` are byte-identical to upstream.
//! - Public API: upstream items are `pub(crate)`; here `render`,
//!   `MermaidStyles`, `MermaidArt`, `Style`, `Color`, and `Modifier` are
//!   `pub` so the markdown pipeline can drive the renderer.
//! - Edition: the workspace is Rust 2021, so upstream's edition-2024
//!   let-chains are written as nested `if let`s; control flow is unchanged.
//! - Algorithm, resource-limit constants, and `plain_lines` output are
//!   otherwise untouched — do not "improve" without re-baselining against
//!   the upstream tests.
...
// ---------------------------------------------------------------------------
// ratatui → ANSI adapter (local shim; the crate has no ratatui dependency)
// ---------------------------------------------------------------------------

/// Foreground color, serialized as ANSI truecolor (`38;2;r;g;b`) or 256-color
/// (`38;5;n`).
...
/// Ratatui `Modifier` subset used by the blueprint (`Italic` on the too-wide
/// hint; `Bold` kept for caller-built styles).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Modifier {
    Bold,
    Italic,
...
/// Ratatui `Style` subset: foreground color plus bold/italic flags.
...
impl Style {
    /// Ratatui `Style::add_modifier` equivalent (blueprint mermaid.rs:3563).
    ...
    /// Open/close SGR sequences. Open order is italic, fg, bold; close order
    /// is the reverse (bold-off, fg-off, italic-off), matching the crate's
    /// snapshot conventions (`…\x1b[22m\x1b[39m`, `…\x1b[39m\x1b[23m`).
    fn sgr(&self) -> (String, String) {
        ...
        match self.fg {
            Some(Color::Rgb(r, g, b)) => open.push_str(&format!("\x1b[38;2;{r};{g};{b}m")),
            Some(Color::Ansi256(n)) => open.push_str(&format!("\x1b[38;5;{n}m")),
            None => {}
        }
        if self.bold {
        ...
        if self.bold {
            close.push_str("\x1b[22m");
        }
        if self.fg.is_some() {
            close.push_str("\x1b[39m");
        }
        if self.italic {
            close.push_str("\x1b[23m");
        }
        (open, close)
    }
}

/// Ratatui `Span` subset: one styled run of text.
struct Span {
    content: String,
    style: Style,
}
...
impl Span {
    /// Ratatui `Span::styled` equivalent.
    fn styled(content: String, style: Style) -> Span {
        Span { content, style }
    }
}

/// Ratatui `Line` subset: one styled row; serializes to a single ANSI string.
struct Line {
    spans: Vec<Span>,
}
...
/// Adapter step at each `MermaidArt` construction site: serialize shim
/// `Line`s to ANSI strings (upstream keeps ratatui `Line`s here).
fn to_ansi_lines(lines: &[Line]) -> Vec<String> {
...
/// Rendered diagram: ANSI-styled lines for the TUI and plain lines for
/// non-ANSI output (adapter: upstream keeps ratatui `Line`s here).
...
pub fn render(src: &str, styles: &MermaidStyles, max_width: Option<usize>) -> Option<MermaidArt> {
...
fn render_class(
    ...
    Ok(MermaidArt {
        styled_lines: to_ansi_lines(&styled_lines),
        plain_lines,
    })
}
...
impl Canvas {
    ...
    fn to_lines(&self, styles: &MermaidStyles) -> (Vec<Line>, Vec<String>) {
...
fn parse_note_anchor(rest: &str, seq: &mut Sequence) -> Option<(String, NoteAnchor)> {
    ...
    let (ids_and_text, kind) = if let Some(r) = lower.strip_prefix("over ") {
        ...
        // Clippy `question_mark` mechanical rewrite of the upstream
        // `else if let Some(r) = … { … } else { return None; }` tail;
        // behavior unchanged.
...
fn fallback(
    ...
    MermaidArt {
        styled_lines: to_ansi_lines(&styled),
        plain_lines: plain,
    }
}
...
mod tests {
    ...
    /// Strip SGR sequences (`\x1b[…m`); the shim emits only SGR, never OSC 8.
    ...
    fn fallback_styled_and_plain_widths_match() {
        ...
        for (styled, plain) in art.styled_lines.iter().zip(&art.plain_lines) {
            // Adapter: styled lines are ANSI-serialized shim lines; stripping
            // the SGR codes recovers exactly the plain row's content.
            assert_eq!(
                strip_ansi(styled).width(),
                plain.width(),
                "styled/plain widths diverge"
            );
