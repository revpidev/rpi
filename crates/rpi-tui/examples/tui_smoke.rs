...
//!
//! ```sh
//! script -qc 'cargo run -p rpi-tui --example tui_smoke' /dev/null
//! ```
...
fn main() {
    ...
    let _signal_task = runtime
        .as_ref()
        .ok()
        .and_then(|_rt| rpi_tui::recovery::spawn_signal_restore(&tui));

    tui.add_child(shared_component(Text::new(
        "rpi-tui smoke demo — press q to quit (auto-exits after 10s)",
        1,
        1,
        None,
    )));
