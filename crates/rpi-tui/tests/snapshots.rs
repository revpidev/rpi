...
//!
//! ```sh
//! RPI_UPDATE_SNAPSHOTS=1 cargo test -p rpi-tui --test snapshots
//! ```
//!
...
use rpi_tui::tui::{Component, Container, Focusable, RenderHandle};
...
impl Terminal for FixedTerminal {
    fn start(
        &mut self,
        _on_input: rpi_tui::terminal::InputHandler,
...
fn editor_at(columns: usize, rows: usize) -> Editor {
    let tui = TuiHandle::from_main(TuiMainScreen::new(Box::new(FixedTerminal::new(
        columns as u16,
        rows as u16,
    ))));
    Editor::new(
...
impl rpi_tui::tui::Component for SubmenuHintStub {
