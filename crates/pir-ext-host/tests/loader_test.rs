...
use pir_ext_host::api::ExtensionRuntime;
use pir_ext_host::loader::{
...
fn loader_discovers_one_level_wasm_files_and_dir_entries() {
    ...
    // 3. subdirectory with pir-extension.json manifest naming its wasm.
    let manifested = temp.write("packed/dist/main.wasm", "wasm");
    temp.write(
        "packed/pir-extension.json",
        r#"{"name": "packed", "wasm": "dist/main.wasm", "pirAbi": 1}"#,
    );
    // manifest naming a missing wasm falls through to index.wasm (none) →
    // no entries.
    temp.write("broken/pir-extension.json", r#"{"wasm": "missing.wasm"}"#);
...
async fn loader_load_order_cli_then_local_then_global_then_packages_then_inline() {
    ...
    let local = temp.write("project/.pir/extensions/local.wasm", "wasm");
    let global = temp.write("agent/extensions/global.wasm", "wasm");
    let package = temp.write("pkg/pkg.wasm", "wasm");

    let runtime = ExtensionRuntime::new();
    let loader = ExtensionLoader::new(runtime);
    let config = pir_ext_host::loader::DiscoverConfig {
        cwd: cwd.clone(),
        agent_dir,
