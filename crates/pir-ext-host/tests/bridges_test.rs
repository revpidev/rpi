...
use pir_ext_host::bridges::NullUiBridge;
use serde_json::json;
...
async fn null_bridge_matches_noop_ui_context() {
    ...
    bridge.notify("m", pir_ext_host::api::NotifyType::Info);
...
async fn unbound_ui_falls_back_to_null_bridge_and_has_ui_false() {
    ...
    let host = pir_ext_host::host::NativeExtensionHost::new("/bridges-cwd");
    ...
    host.runtime().set_ui_bridge(
        ...
        pir_ext_host::types::ExtensionMode::Print,
    );
