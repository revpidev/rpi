...
//! exported `pir_dispatch` function). Differences from the wasm guest:
...
/// The host-call handle bundle handed to `pir_extension_init` BY VALUE —
...
/// `#[export_root_module]` (see `crates/pir-test-native-plugin`).
#[repr(C)]
#[derive(StableAbi)]
#[sabi(kind(Prefix(prefix_ref = PirNativeModule_Ref)))]
#[sabi(missing_field(panic))]
pub struct PirNativeModule {
    /// Load entry: registers via the host-call handle, returns the init
    /// receipt JSON (`{"ok": true}` / `{"error": {...}}`).
    pub pir_extension_init: extern "C" fn(PirHostCalls, PluginCookie) -> RVec<u8>,
    /// Dispatch entry (event/toolExecute/command/shortcut/render/bus).
    #[sabi(last_prefix_field)]
    pub pir_dispatch: extern "C" fn(PluginCookie, RVec<u8>) -> RVec<u8>,
}

impl RootModule for PirNativeModule_Ref {
    abi_stable::declare_root_module_statics! {PirNativeModule_Ref}
...
/// module, run `pir_extension_init`, keep the handles on the extension.
pub async fn load_native_plugin(
    path: &Path,
    api: ExtensionApi,
    capabilities: HashSet<Capability>,
) -> Result<(), String> {
    let module = PirNativeModule_Ref::load_from_file(path)
        .map_err(|e| format!("load dynamic library {}: {e}", path.display()))?;
    let dispatch_fn = module.pir_dispatch();
    let init_fn = module.pir_extension_init();
