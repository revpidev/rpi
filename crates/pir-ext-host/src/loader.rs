...
//!   `pir-extension.json` manifests (design §13 open item 2) instead of
...
/// an extension when it has a `pir-extension.json` manifest naming a `wasm`
/// file that exists, or an `index.wasm`.
fn resolve_extension_entries(dir: &Path) -> Option<Vec<PathBuf>> {
    let manifest_path = dir.join("pir-extension.json");
...
pub struct DiscoverConfig {
    /// Working directory (`.pir/extensions` discovery root).
    pub cwd: PathBuf,
    /// Agent dir (`~/.pir/agent`; its `extensions/` subdirectory is
    /// discovered).
    ...
    /// Include project-local `.pir/extensions` discovery. `false` under an
...
impl ExtensionLoader {
    ...
    /// 2. project-local `.pir/extensions/` (when included)
    ...
    pub fn resolve_paths(config: &DiscoverConfig) -> Vec<PathBuf> {
        ...
        if !config.no_extensions {
            ...
            if config.include_project_local {
                let local_dir = config.cwd.join(".pir").join("extensions");
                add_paths(
...
/// `pir-extension.json` (directory-level manifest).
#[derive(Debug, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct WasmManifest {
    // name/version/wasm/description are discovery/display metadata; the W6
    // loader only enforces `pirAbi` + `capabilities` (display lands with
...
/// containing `pir-extension.json` within two levels above the file (the
...
fn find_manifest(wasm_path: &Path) -> Option<PathBuf> {
    ...
    for _ in 0..2 {
        let current = dir?;
        let candidate = current.join("pir-extension.json");
...
/// `pirAbi` and map capability strings.
fn wasm_capabilities_for(
    ...
    match manifest.pir_abi {
        Some(crate::wasm::PIR_ABI_VERSION) => {}
        other => {
            return Err(format!(
                "Failed to load extension: unsupported pirAbi {other:?} (host implements {})",
                crate::wasm::PIR_ABI_VERSION
            ))
        }
    }
