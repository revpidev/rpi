...
//!   `pir_dealloc(ptr: u32, len: u32)`, `pir_extension_init() -> u64`,
//!   `pir_dispatch(ptr: u32, len: u32) -> u64`;
//! - host import (module `pir`): `pir_host_call(ptr: u32, len: u32) -> u64`;
...
/// ABI version this host implements (`pirAbi` manifest field).
pub const PIR_ABI_VERSION: u32 = 1;
...
pub(crate) enum GuestCommand {
    /// Run `pir_extension_init`; respond with the receipt JSON.
    Init(std::sync::mpsc::Sender<Result<Value, String>>),
    /// Run `pir_dispatch` with a JSON message; respond with its result.
...
impl DispatchTarget {
    /// `pir_dispatch`-equivalent round trip (serial per extension).
...
impl WasmForward {
    /// `pir_dispatch` round trip (serial per guest).
    pub async fn dispatch(&self, message: Value, command_context: bool) -> Result<Value, String> {
        let bytes = serde_json::to_vec(&message).map_err(|e| e.to_string())?;
...
/// `pir_extension_init`. The receipt JSON (`{"ok":...}` /
...
pub async fn instantiate_and_init(
    ...
    let join = std::thread::spawn(move || {
        ...
        if let Err(error) = linker.func_wrap("pir", "pir_host_call", host_call) {
        ...
        let init = match instance.get_typed_func::<(), u64>(&mut store, "pir_extension_init") {
            Ok(func) => func,
            Err(error) => {
                let _ = ready_tx.send(Err(format!("missing pir_extension_init: {error}")));
                return;
            }
        };
        let dispatch = match instance.get_typed_func::<(u32, u32), u64>(&mut store, "pir_dispatch")
        {
            Ok(func) => func,
            Err(error) => {
                let _ = ready_tx.send(Err(format!("missing pir_dispatch: {error}")));
                return;
            }
        };
    ...
    // `pir_extension_init` — the receipt decides the load outcome.
