...
//!
//! [`HostActions`] / [`UiBridge`] are defined here (pir-ext-host) and
//! implemented by the `pir` crate per mode (W3/W4); the W1 host runs
...
/// `Model`.
///
/// Implemented by the `pir` crate in W3 (`bindCore`, runner.ts:311-408).
...
pub trait HostActions: Send + Sync {
    ...
    /// (loader.ts:374-382): a pir-ai `Provider` trait object. Only native
...
/// receives a pir `SessionManager`) — v1 deviation: use `with_session`.
...
pub struct PendingNativeProviderRegistration {
    pub provider: Arc<dyn pir_ai::models::Provider>,
    pub extension_path: String,
}
...
impl ReplacedSessionContext {
    /// Built by the pir side after a session replacement (the new
...
impl LoadedExtension {
    ...
    /// Attach the guest instance after a wasm `pir_extension_init`
    /// succeeds (T15 W6).
