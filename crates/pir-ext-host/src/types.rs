...
use pir_agent::types::{AgentToolResult, AgentToolUpdateCallback, ToolExecutionMode};
use pir_ai::types::{ImageContent, ProviderHeaders, Usage, UserContent};
...
/// input examples; the pir-side seam (`pir::core::extensions::InputSource`)
...
/// upstream `CompactionResult` JSON (pir-agent's `CompactionResult`
...
pub struct TurnEndEvent {
    ...
    pub tool_results: Vec<pir_ai::types::ToolResultMessage>,
}
...
/// upstream `Model` JSON (pir-ai `Model` serializes to that shape).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// pir-tui components (`pir::modes::interactive::component_tree`).
...
///
/// v1 deviations from the design sketch (§13): `row` is deferred — pir-tui
...
/// [`ComponentTree`] instead of a pir-tui `Component` (see [`ComponentTree`]).
...
/// component description wasm guests produce — rather than pir-tui
...
/// item. Duplicated here (rather than reusing pir's) because the dependency
/// direction is `pir` → `pir-ext-host`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// (JSON; the pir-tui `AutocompleteItem` mapping lands in W4) or `None`.
