...
//! surface the `pir` crate drives @ pi 0.82.1 (2efa728).
//!
//! Upstream counterpart: the `ExtensionRunner` (extensions/runner.ts) plus
//! the loading half of `DefaultResourceLoader`
//! (resource-loader.ts:494-560). The pir-side adapter
//! (`pir::core::extension_host_adapter`) maps this host onto the session's
...
impl NativeExtensionHost {
    ...
    /// `include_project_local` gates `.pir/extensions` discovery on the
    ...
    // ========================================================================
    // Emit surface (called by the pir-side adapter / emit sites)
    // ========================================================================
