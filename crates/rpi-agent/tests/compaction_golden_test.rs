...
//! byte-by-byte against the `rpi_agent::compaction` port.
//!
...
use rpi_agent::stream_fn::BoxStream;
use rpi_ai::types::{
...
fn llm_messages(value: &Value) -> Vec<rpi_ai::types::Message> {
...
fn golden_is_context_overflow() {
    ...
    for case in golden["isContextOverflow"].as_array().expect("array") {
        ...
        assert_eq!(
            rpi_ai::utils::overflow::is_context_overflow(
                &message,
...
fn captured_prompt(capture: &Capture, index: usize) -> String {
    ...
    let Some(rpi_ai::types::UserContentBlock::Text(text)) = blocks.first() else {
        panic!("text block");
    };
    text.text.clone()
}
