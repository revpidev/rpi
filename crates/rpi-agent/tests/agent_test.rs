...
fn done_result(status: &str, terminate: bool) -> Result<AgentToolResult, rpi_agent::AgentError> {
    Ok(AgentToolResult {
...
async fn forwards_thinking_level_to_stream_fn_options() {
    ...
    let received: Arc<Mutex<Vec<Option<rpi_ai::types::ModelThinkingLevel>>>> =
    ...
    // xhigh → Some(Xhigh).
    agent.set_thinking_level(rpi_agent::types::ThinkingLevel::Xhigh);
