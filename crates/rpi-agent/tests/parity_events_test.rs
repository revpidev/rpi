...
//! the `rpi-agent` `Agent` produces under the same script.
//!
...
//!   part of the contract; the rpi-side faux chunks deterministically.
...
//!   line-order-sensitive JSONL-diffed after `rpi_test_support`
...
use rpi_agent::types::{AgentEvent, AgentTool, AgentToolResult, AgentToolUpdateCallback};
...
type FixtureExecuteFn = Arc<
    dyn Fn(
        ...
        ) -> BoxFuture<'static, Result<AgentToolResult, rpi_agent::AgentError>>
        + Send
        + Sync,
>;
...
fn text_tool_result(text: &str) -> Result<AgentToolResult, rpi_agent::AgentError> {
    Ok(AgentToolResult {
