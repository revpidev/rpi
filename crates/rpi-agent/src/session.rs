...
//! source shared by the `rpi` main path and the `rpi-agent` harness layer.
//!
...
//!   enforced by the writers, not the type: the `rpi` main path writes only
...
pub struct CustomMessageEntry {
    ...
    #[serde(default)]
    pub content: rpi_ai::types::UserContent,
    pub display: bool,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub details: Option<Value>,
}
...
/// before.
///
/// Single source for this parse: the `rpi` main path (`session_manager.rs`)
...
mod tests {
    ...
    fn remaining_entry_type_shapes() {
        ...
        let custom_message = FileEntry::CustomMessage(CustomMessageEntry {
            ...
            content: rpi_ai::types::UserContent::Text("hello".to_owned()),
            display: false,
            details: None,
        });
        assert_eq!(
