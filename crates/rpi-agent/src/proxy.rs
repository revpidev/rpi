...
//!   `rpi_ai::api::sse::SseDecoder`: the proxy treats every `data: ` line as
...
//!   map (`ToolCall` has no such field in rpi); the JS quirk of
...
#[cfg(test)]
mod tests {
    use rpi_ai::types::{ApiKind, InputModality, ModelCost};
    use serde_json::json;
