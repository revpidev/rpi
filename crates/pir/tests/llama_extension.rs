...
use pir::extensions::llama::client::{LlamaClient, LlamaModelInfo, LlamaProgress};
use pir::extensions::llama::huggingface::{find_hugging_face_token, HuggingFaceClient};
use pir::extensions::llama::provider::{create_llama_provider, LLAMA_PROVIDER_ID};
use pir::extensions::llama::{
    ConnectionErrorChoice, HuggingFaceSearchFn, LlamaHost, LlamaManagerAction, LlamaUi,
    NotifyLevel, ProgressState,
};
use pir_ai::auth::{AuthResult, Credential, ModelAuth, ModelsError};
use pir_ai::models::{RefreshModelsContext, ScopedModelsStore};
use pir_ai::models_store::{InMemoryModelsStore, ModelsStore, ProviderModelsStore};
use pir_ai::types::ProviderEnv;
...
async fn persists_and_restores_loaded_models_for_cache_only_startup_refreshes() {
    ...
    let credential = Credential::ApiKey(pir_ai::auth::ApiKeyCredential {
...
impl pir_ai::auth::AuthInteraction for ScriptedInteraction {
    fn prompt<'a>(
        &'a self,
        prompt: pir_ai::auth::AuthPrompt,
    ) -> pir_ai::auth::types::BoxFutureSend<'a, Result<String, ModelsError>> {
        Box::pin(async move {
            let message = match &prompt {
                pir_ai::auth::AuthPrompt::Text { message, .. }
                | pir_ai::auth::AuthPrompt::Secret { message, .. }
                | pir_ai::auth::AuthPrompt::ManualCode { message, .. } => message.clone(),
                pir_ai::auth::AuthPrompt::Select { message, .. } => message.clone(),
            };
            self.prompts
                .lock()
                .unwrap_or_else(|e| e.into_inner())
            ...
            self.answers
                ...
                .unwrap_or_else(|e| e.into_inner())
                .pop_front()
                .ok_or_else(|| ModelsError::new(pir_ai::auth::ModelsErrorCode::Auth, "no answer"))
        })
    }

    fn notify(&self, _event: pir_ai::auth::AuthEvent) {}
}
...
async fn login_stores_url_plus_optional_key() {
    ...
    let context = pir_ai::auth::DefaultAuthContext;
    let resolved = auth
...
fn model_action(id: &str, status: &str) -> LlamaManagerAction {
    LlamaManagerAction::Model(Box::new(LlamaModelInfo {
        id: id.to_owned(),
        status: pir::extensions::llama::client::LlamaModelStatus {
...
async fn non_tui_mode_only_warns() {
    ...
    pir::extensions::llama::run_llama_manager(
        &host,
        &controller,
        &mut ui,
...
fn parse_hugging_face_model_splits_quant() {
    use pir::extensions::llama::parse_hugging_face_model;
    assert_eq!(
