//! `pir --export <file> [output.html]` CLI contract test (main.ts:526-538):
...
fn run_export(dir: &Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_pir"))
        .args(args)
        .current_dir(dir)
...
fn export_default_output_name() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Exported to: pir-session-abc.html"),
        "stdout: {stdout}"
    );
    assert!(tmp.path().join("pir-session-abc.html").exists());
}
