...
fn load_discovered(options: &DiscoverSkillsOptions) -> pir::core::skills::LoadSkillsResult {
...
fn skills_discovery_pir_and_agents_modes_end_to_end() {
    ...
    // `.pir/skills` (mode "pi"): loose root .md files count as skills, and
    // SKILL.md directories are skill roots (no recursion past them).
    write(
        &cwd.join(".pir/skills/loose.md"),
        "---\nname: loose-pi\ndescription: pi loose\n---\n",
    );
    write(
        &cwd.join(".pir/skills/dir-skill/SKILL.md"),
        &skill_md("pi dir"),
    );
    write(
        &cwd.join(".pir/skills/dir-skill/nested/SKILL.md"),
...
fn skills_discovery_respects_fdignore_chain() {
    let tmp = TempDir::new();
    let cwd = tmp.path().join("project");
    write(&cwd.join(".pir/skills/.fdignore"), "hidden-skill/\n");
    write(
        &cwd.join(".pir/skills/hidden-skill/SKILL.md"),
        &skill_md("hidden"),
    );
    write(
        &cwd.join(".pir/skills/visible/SKILL.md"),
        &skill_md("visible"),
    );
...
fn skills_rank_project_settings_beats_project_auto_on_name_collision() {
    let tmp = TempDir::new();
    let cwd = tmp.path().join("project");
    let settings_skill = cwd.join(".pir/declared/dup/SKILL.md");
    let auto_skill = cwd.join(".pir/skills/dup/SKILL.md");
    write(
        &settings_skill,
...
fn skills_trust_gate_excludes_project_auto_discovery() {
    ...
    let cwd = tmp.path().join("project");
    write(
        &cwd.join(".pir/skills/s/SKILL.md"),
...
fn skills_frontmatter_field_semantics() {
    let tmp = TempDir::new();
    let cwd = tmp.path().join("project");
    let skills_dir = cwd.join(".pir/skills");
...
fn skills_xml_injection_gate_and_disable_model_invocation() {
    ...
    write(
        &cwd.join(".pir/skills/hidden/SKILL.md"),
...
fn skills_xml_block_exact_shape_from_disk() {
    let tmp = TempDir::new();
    let cwd = tmp.path().join("project");
    let skill_file = cwd.join(".pir/skills/exact/SKILL.md");
    write(
        &skill_file,
...
fn skills_expand_command_exact_format_with_args() {
    let tmp = TempDir::new();
    let cwd = tmp.path().join("project");
    let skill_file = cwd.join(".pir/skills/deploy/SKILL.md");
    write(
        &skill_file,
