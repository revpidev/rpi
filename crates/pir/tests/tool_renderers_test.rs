...
use pir::core::themes::load_theme;
...
fn theme() -> Arc<pir::core::themes::Theme> {
...
fn write_streaming_preview_grows_but_stays_clamped() {
    ...
    for round in 0..5usize {
        ...
        component.update_args(json!({"path": "/tmp/pir.txt", "content": content}));
        let lines = component.render(80);
        let out = pir_test_support::vt::strip_ansi(&lines.join("\n"));
        assert!(
            out.contains("write /tmp/pir.txt"),
            "round {round} out: {out}"
        );
        assert!(!out.contains("\\n"), "round {round} out: {out}");
    ...
    component.update_result(
        ToolResultState {
            content: vec![ToolResultContentLoose::text(
                "Successfully wrote 250 bytes to /tmp/pir.txt",
            )],
            is_error: false,
            details: None,
        },
