...
impl TempDir {
    fn new() -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("pir-ext-w6e2e-{}-{id}", std::process::id()));
...
(module
  ...
  (func (export "pir_dispatch") (param $ptr i32) (param $len i32) (result i64)
...
async fn e2e_fixture(responses: Vec<FauxResponseStep>, wasm_wat: Option<&str>) -> E2eFixture {
    ...
    if let Some(wat) = wasm_wat {
        let pkg = cwd.join(".pir/extensions/parity");
        ...
        std::fs::write(
            ...
            r#"{"name":"parity","version":"0.1.0","wasm":"dist/guest.wasm","capabilities":["tools","session"],"pirAbi":1}"#,
        )
        .expect("manifest");
    }
    let errors = host
...
fn first_tool_result(session: &pir::core::agent_session::AgentSession) -> Value {
    session
        .messages()
        .into_iter()
