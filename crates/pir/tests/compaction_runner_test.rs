...
use pir_ai::types::{AssistantMessage, StopReason, Usage};
...
async fn check_compaction_silent_overflow_compacts_without_retry() {
    ...
    match &events[1] {
        CompactionEvent::CompactionEnd {
            ...
            assert_eq!(
                *reason,
                pir::core::compaction_runner::CompactionReason::Overflow
            );
            assert!(!will_retry);
            assert!(!aborted);
...
async fn check_compaction_threshold_estimates_from_last_valid_usage() {
    ...
    match &events[0] {
        CompactionEvent::CompactionStart { reason } => {
            assert_eq!(
                *reason,
                pir::core::compaction_runner::CompactionReason::Threshold
            );
        }
