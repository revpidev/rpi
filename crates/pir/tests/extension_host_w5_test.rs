...
use pir::core::agent_session::ExtensionBindings;
use pir::core::extension_actions::bind_session_actions;
use pir::core::extension_context::RuntimeCommandActions;
use pir_ext_host::api::{CommandContextActions, CompactOptions, ExtensionApi, ExtensionContext};
use pir_ext_host::host::NativeExtensionHost;
use pir_ext_host::loader::InlineExtension;
...
impl TempDir {
    fn new() -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("pir-ext-w5-test-{}-{id}", std::process::id()));
...
struct RuntimeFixture {
    runtime: pir::core::agent_session_runtime::AgentSessionRuntime,
...
async fn runtime_fixture(event_log: Arc<Mutex<Vec<String>>>) -> RuntimeFixture {
    ...
    let runtime = pir::core::agent_session_runtime::create_agent_session_runtime(
        Arc::new(factory),
...
async fn w5_context_compact_fires_and_forgets_with_callback() {
    ...
    // 铺三轮对话（超过 keep_recent 阈值）使压缩有内容。
    {
        ...
        for i in 0..3 {
            manager
                .append_message(pir_agent::messages::AgentMessage::User(
                    pir_ai::types::UserMessage {
                        role: pir_ai::types::UserRole::User,
                        content: pir_ai::types::UserContent::Text(format!(
                            "question {i} {}",
                            "x".repeat(80)
                        )),
                        timestamp: 1,
            ...
            manager
                .append_message(pir_agent::messages::AgentMessage::Assistant(assistant))
                .expect("assistant");
        }
    ...
    let target_id = {
        ...
        let first = manager
            .append_message(pir_agent::messages::AgentMessage::User(
                pir_ai::types::UserMessage {
                    role: pir_ai::types::UserRole::User,
                    content: pir_ai::types::UserContent::Text("q1".to_owned()),
                    timestamp: 1,
                },
            ))
            .expect("user");
        manager
            .append_message(pir_agent::messages::AgentMessage::User(
                pir_ai::types::UserMessage {
                    role: pir_ai::types::UserRole::User,
                    content: pir_ai::types::UserContent::Text("q2".to_owned()),
                    timestamp: 2,
                },
            ))
            .expect("user2");
