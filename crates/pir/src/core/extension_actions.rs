...
pub async fn bind_session_actions(
    host: &Arc<pir_ext_host::host::NativeExtensionHost>,
...
impl HostActions for SessionHostActions {
    ...
    async fn set_model(&self, model: Value) -> bool {
        ...
        let model: pir_ai::types::Model = match serde_json::from_value(model) {
            Ok(model) => model,
            Err(error) => {
    ...
    async fn register_provider(&self, name: &str, config: Value) -> Result<(), String> {
        ...
        for key in ["streamSimple", "oauth", "refreshModels"] {
            if config.get(key).is_some_and(|v| !v.is_null()) {
                return Err(format!(
                    "ProviderConfig.{key} is not supported by the pir host (T15 candidate deviation)"
                ));
            }
        }
    ...
    async fn register_native_provider(
        &self,
        provider: Arc<dyn pir_ai::models::Provider>,
    ) -> Result<(), String> {
        let Some(session) = self.session() else {
            return Err("session is gone".to_owned());
        };
...
fn thinking_level_str(level: pir_agent::types::ThinkingLevel) -> &'static str {
    match level {
        pir_agent::types::ThinkingLevel::Off => "off",
        pir_agent::types::ThinkingLevel::Minimal => "minimal",
        pir_agent::types::ThinkingLevel::Low => "low",
        pir_agent::types::ThinkingLevel::Medium => "medium",
        pir_agent::types::ThinkingLevel::High => "high",
        pir_agent::types::ThinkingLevel::Xhigh => "xhigh",
        pir_agent::types::ThinkingLevel::Max => "max",
    }
}
