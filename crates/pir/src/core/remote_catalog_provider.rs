...
//! - The `catalogBaseUrl` default is `https://pi.dev`; settings/env
...
use pir_ai::models::{merge_models, now_millis, InflightRefresh, Provider, RefreshModelsContext};
use pir_ai::models_store::ModelsStoreEntry;
use pir_ai::types::{Context, Model, ProviderHeaders, SimpleStreamOptions, StreamOptions};
...
/// {runtime}; {arch})` with the pir naming (ADR-0001) and a `rust` runtime
/// marker (D-036).
...
mod tests {
    ...
    fn test_provider(
        ...
        let inner: Arc<dyn Provider> =
            pir_ai::models::create_provider(pir_ai::models::CreateProviderOptions {
                id: "test-provider".to_owned(),
                name: None,
                base_url: None,
                headers: None,
                auth: pir_ai::auth::ProviderAuth {
                    api_key: Some(Arc::new(pir_ai::auth::helpers::env_api_key_auth(
                        "Test API key",
                        &["PIR_TEST_REMOTE_CATALOG_KEY"],
                    ))),
                    oauth: None,
                },
                models: vec![model("static")],
                api: pir_ai::models::ProviderApi::Single(Arc::new(
    ...
    async fn parses_keyed_catalogs_sends_ua_and_observes_ttl_and_force() {
        ...
        assert!(
            ua.contains(&format!("pir/{VERSION}")),
            "user-agent {ua} missing pir/{VERSION}"
        );
    }
