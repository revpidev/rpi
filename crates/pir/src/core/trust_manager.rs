...
/// `formatProjectTrustPrompt` (project-trust.ts:24-26), rebranded to pir.
pub fn format_project_trust_prompt(cwd: &Path) -> String {
    format!(
        "Trust project folder?\n{}\n\nThis allows pir to load {CONFIG_DIR_NAME} settings and resources, install missing project packages, and execute project extensions.",
        cwd.display()
    )
}
...
mod tests {
    ...
    /// A cwd that requires trust (`.pir/settings.json` present).
