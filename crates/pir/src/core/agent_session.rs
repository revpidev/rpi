...
use pir_agent::compaction::{estimate_context_tokens, CompactionResult, SummarizationArgs};
use pir_agent::messages::{AgentMessage, BashExecutionMessage, CustomMessage, CustomRole};
...
use pir_ai::utils::retry::{is_retryable_assistant_error, RetryPolicy};
use pir_ai::utils::text::content_text_user;
use serde::Serialize;
...
/// The pir-agent placeholder model represents upstream `undefined`
...
fn builtin_tool_guidelines(name: &str) -> &'static [&'static str] {
    match name {
        ...
        "bash" => &["Inspect PIR_* environment variables for current model and session details."],
        "edit" => &[
...
impl AgentSession {
    ...
    fn sync_compaction_model(&self) {
        ...
        if let Ok(mut runner) = self.inner.compaction.try_lock() {
            ...
            runner.set_settings(pir_agent::compaction::CompactionSettings {
    ...
    pub fn record_bash_result(
        ...
        let message = BashExecutionMessage {
            role: pir_agent::messages::BashExecutionRole::BashExecution,
                    ...
                    match &message_entry.message {
                        ...
                        AgentMessage::Assistant(assistant) => {
                            ...
                            tool_calls += assistant
                                ...
                                .iter()
                                .filter(|c| {
                                    matches!(c, pir_ai::types::AssistantContent::ToolCall(_))
                                })
                                .count() as u64;
    ...
    pub fn get_context_usage(&self) -> Option<ContextUsage> {
        ...
        let typed: Vec<SessionEntry> = branch_entries
            ...
            .filter_map(|entry| entry.known().cloned())
            .collect();
        let latest_compaction = pir_agent::session::get_latest_compaction_entry(&typed);
                ...
                for entry in typed.iter().skip(index + 1).rev() {
                    if let SessionEntry::Message(message_entry) = entry {
                        if let AgentMessage::Assistant(assistant) = &message_entry.message {
                            ...
                            {
                                let context_tokens =
                                    pir_ai::utils::estimate::calculate_context_tokens(
                                        &assistant.usage,
                                    );
                                if context_tokens > 0 {
    ...
    pub fn export_to_jsonl(&self, output_path: Option<&str>) -> Result<PathBuf, PirError> {
        ...
        let session = lock(&self.inner.session_manager);
        let header = pir_agent::session::FileEntry::Session(pir_agent::session::SessionHeader {
            version: Some(pir_agent::session::CURRENT_SESSION_VERSION),
    ...
    pub fn get_last_assistant_text(&self) -> Option<String> {
        ...
        for content in &assistant.content {
            if let pir_ai::types::AssistantContent::Text(text_content) = content {
...
) -> pir_ai::utils::retry::RetryCallbacks {
    use pir_ai::utils::retry::RetryCallbacks;
