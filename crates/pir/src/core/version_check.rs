...
//! (2efa728) — the "latest release" probe behind `pir update --self`.
//!
//! Intentional differences:
//! - `PI_OFFLINE` → `PIR_OFFLINE` (ADR-0001); the offline check reuses the
...
//! - `getPiUserAgent` carries the pir naming and a `rust` runtime marker
//!   (ADR-0001 / D-038 precedent).
//! - T14-W6a (ADR-0002 §8): the endpoint URL is configurable via the
//!   `PIR_VERSION_CHECK_URL` env var or the `versionCheckUrl` setting
//!   ([`version_check_endpoint`], pir-specific — upstream hardcodes
...
/// `getPiUserAgent` (utils/pi-user-agent.ts): `pir/{version} ({platform};
/// rust; {arch})`.
pub fn pir_user_agent(version: &str) -> String {
    format!(
        "pir/{version} ({}; rust; {})",
        std::env::consts::OS,
        std::env::consts::ARCH
...
/// timeout; `PIR_OFFLINE` short-circuits to `None`.
...
pub async fn get_latest_pir_release_with(
    ...
    let Some(body) = transport
        .get(url, &pir_user_agent(current_version), timeout)
        .await?
    else {
        return Ok(None);
    };
...
/// Resolve the version-check endpoint: `PIR_VERSION_CHECK_URL` env >
/// `versionCheckUrl` setting > [`LATEST_VERSION_URL`]; the literal `off`
/// disables the check (`None` — callers must not probe). Pir-specific
...
/// endpoint: `PIR_SKIP_VERSION_CHECK` or offline or a disabled endpoint all
...
/// (`PIR_SKIP_VERSION_CHECK` / `PIR_OFFLINE`).
...
mod tests {
    ...
    async fn parses_version_package_name_and_note() {
        let transport = ScriptedTransport::responds(Ok(Some(
            r#"{"version": " 1.2.3 ", "packageName": "pir-next", "note": "  hi  "}"#.to_string(),
        )));
        let release = get_latest_pir_release("1.0.0", &transport)
            .await
            .expect("release")
        ...
        assert_eq!(release.package_name.as_deref(), Some("pir-next"));
        ...
        assert_eq!(calls[0].1, pir_user_agent("1.0.0"));
    }
    ...
    fn startup_probe_url_composes_the_gates() {
        let endpoint = Some("https://pi.dev/api/latest-version".to_string());
        assert_eq!(
            startup_probe_url(false, false, endpoint.clone()).as_deref(),
            Some("https://pi.dev/api/latest-version")
        );
    ...
    /// Read-only env use: no test writes `PIR_VERSION_CHECK_URL` (the
