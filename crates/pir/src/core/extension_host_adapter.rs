//! Adapter: `pir_ext_host::NativeExtensionHost` → the session-facing
...
//! `pir-ext-host`; this module only converts types between the pir-side
...
use pir_ext_host::types::{self as ext, ExtensionError};
...
/// Parse a host `project_trust` result JSON into the pir-side result type.
...
/// Convert the host's `ExtSourceInfo` (source-info.ts shape) into pir's
...
#[async_trait]
impl pir_agent::types::AgentTool for HostToolAdapter {
    fn name(&self) -> &str {
    ...
    fn execution_mode(&self) -> Option<pir_agent::types::ToolExecutionMode> {
    ...
    async fn execute(
        ...
        on_update: Option<pir_agent::types::AgentToolUpdateCallback>,
    ) -> Result<pir_agent::types::AgentToolResult, pir_agent::AgentError> {
        ...
        let mut result = (self.definition.execute)(request, ctx)
            .await
            .map_err(pir_agent::AgentError::Tool)?;
...
impl ExtensionRunner for ExtensionHostAdapter {
    ...
    async fn emit_tool_result(
        ...
        tool_name: &str,
        input: Value,
        content: &[pir_ai::types::ToolResultContent],
        details: &Value,
        is_error: bool,
        usage: Option<&pir_ai::types::Usage>,
    ...
    async fn emit_user_bash(
        ...
        if result.get("operations").is_some_and(|o| !o.is_null()) {
            ...
            tracing::warn!(
                "user_bash extension returned custom operations; not supported by the pir host, ignoring"
            );
        }
