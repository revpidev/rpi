...
use pir_agent::session::{CompactionEntry, SessionEntry};
use pir_ai::types::{
...
fn preserves_tool_and_summary_usage_across_a_file_backed_reload() {
    ...
    let usage = usage_sample();
    session
        .append_message(AgentMessage::ToolResult(pir_ai::types::ToolResultMessage {
            role: pir_ai::types::ToolResultRole::ToolResult,
            tool_call_id: "call-1".to_owned(),
            tool_name: "nested-model".to_owned(),
            content: vec![pir_ai::types::ToolResultContent::Text(TextContent {
