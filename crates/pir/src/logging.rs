...
//! - default level `INFO`, overridable via `PIR_LOG` (preferred) or
//!   `RUST_LOG` (standards §16.3);
//! - stderr sink for print/json/rpc modes;
//! - file sink (`<agent_dir>/logs/pir.log`) while the interactive TUI runs
...
impl LogSink {
    ...
    ///
    /// `tui_mode`: file sink under `<agent_dir>/logs/pir.log` (created on
    ...
    pub fn install(tui_mode: bool) -> LogSink {
        ...
        let filter = std::env::var("PIR_LOG")
            .ok()
            .map(EnvFilter::new)
...
/// `<agent_dir>/logs/pir.log`, parent created on demand.
