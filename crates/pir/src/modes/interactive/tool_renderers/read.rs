...
//! - `getPiDocsClassification` anchors at pir's own package root
...
/// lives in the pi/pir package's own `README.md` / `docs/` / `examples/`.
...
mod tests {
    ...
    fn result_image_blocks_render_fallback_text_without_caps() {
        ...
        pir_tui::terminal_image::set_capabilities(pir_tui::terminal_image::TerminalCapabilities {
            images: None,
            true_color: true,
            hyperlinks: false,
        });
    ...
    fn result_image_blocks_skip_indicator_with_kitty_caps() {
        ...
        let _guard = CAPS_LOCK.lock().unwrap();
        pir_tui::terminal_image::set_capabilities(pir_tui::terminal_image::TerminalCapabilities {
            images: Some(pir_tui::terminal_image::ImageProtocol::Kitty),
            true_color: true,
            hyperlinks: false,
        });
        let theme = theme();
