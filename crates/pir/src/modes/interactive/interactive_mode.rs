...
//!   pir-tui lock contract, tui.rs header note).
...
use pir_agent::session::{parse_iso8601_ms, SessionEntry};
...
use pir_tui::keybindings as tui_keybindings;
use pir_tui::tui::{
...
/// config name is set, else the app name). Pir uses `APP_NAME` (ADR-0001).
const APP_TITLE: &str = "pir";
...
/// (interactive-mode.ts:468-469): the pir core global (73-entry table,
/// `app.*` included — read by the keybinding-hint helpers) and the pir-tui
/// global (read by `CustomEditor` dispatch and the editor's `tui.*`
/// matching). The pir core manager owns the definition table, JSON loading
/// and legacy-name migration (core/keybindings.rs); the pir-tui manager is
...
pub(crate) enum UiCommand {
    ...
    CompactionEnd {
        ...
        result: Option<Box<pir_agent::compaction::CompactionResult>>,
        aborted: bool,
        will_retry: bool,
...
fn tool_result_state_from_message(
    tool_result: &pir_ai::types::ToolResultMessage,
    is_error: bool,
) -> ToolResultState {
    ToolResultState {
        content: tool_result
...
impl InteractiveUi {
    ...
    fn streaming_stop_reason_is_error(&self, assistant: &pir_ai::types::AssistantMessage) -> bool {
    ...
    pub(crate) fn maybe_show_cache_miss_notice(
        &self,
        _assistant: &pir_ai::types::AssistantMessage,
    ) {
    ...
    pub(crate) fn show_status(&self, message: &str) {
        ...
        let spacer = Box::new(pir_tui::components::spacer::Spacer::new(1));
    ...
    fn add_message_to_chat(&self, message: AgentMessage, populate_history: bool) {
        match &message {
            ...
            AgentMessage::User(user_message) => {
                let text_content =
                    pir_ai::utils::text::content_text_user(&user_message.content, "");
                ...
                if !chat.children.is_empty() {
                    chat.children
                        .push(Box::new(pir_tui::components::spacer::Spacer::new(1)));
                }
    ...
    fn render_project_trust_warning_if_needed(&self) {
        ...
        chat.children.push(Box::new(Text::new(
            lock(&self.theme).fg(
                "warning",
                &format!(
                    "This project is not trusted. Project {} resources and packages are ignored. Use /trust to save a trust decision, then restart pir.",
...
impl InteractiveMode {
    ...
    pub fn new(runtime: AgentSessionRuntime, options: InteractiveModeOptions) -> Self {
        Self::with_terminal(
            runtime,
            options,
            Box::new(pir_tui::terminal::ProcessTerminal::new()),
        )
    }
    ...
    async fn init_auto_theme(&self) {
        ...
        ui.on_terminal_color_scheme_change(Box::new(move |scheme| {
            ...
            let name = match scheme {
                pir_tui::terminal_colors::TerminalColorScheme::Light => light,
                pir_tui::terminal_colors::TerminalColorScheme::Dark => dark,
            };
...
mod tests {
    ...
    use pir_ai::types::{ApiKind, UserMessage, UserRole};
    ...
    fn text_content(text: &str) -> AssistantContent {
        AssistantContent::Text(pir_ai::types::TextContent {
            text: text.to_string(),
            text_signature: None,
        })
    }
        ...
        let events: Vec<(AgentSessionEvent, &str)> = vec![
            ...
            (
                AgentSessionEvent::Agent(Box::new(AgentEvent::MessageUpdate {
                    ...
                    assistant_message_event: Box::new(pir_ai::types::StreamEvent::TextStart {
                        content_index: 0,
            ...
            (
                AgentSessionEvent::Session(SessionEvent::EntryAppended {
                    entry: Box::new(SessionEntry::Label(pir_agent_entry())),
                }),
                "entry_appended",
            ),
            (
    ...
    fn pir_agent_entry() -> pir_agent::session::LabelEntry {
        pir_agent::session::LabelEntry {
            id: "l1".to_string(),
            parent_id: None,
    ...
    async fn compaction_end_with_result_rebuilds_chat() {
        ...
        *lock(&ui.tool_output_expanded) = true;
        ui.push(UiCommand::CompactionEnd {
            reason: CompactionReason::Threshold,
            result: Some(Box::new(pir_agent::compaction::CompactionResult {
    ...
    async fn render_initial_messages_builds_chat_from_entries() {
        ...
        let (ui, session) = (&mode.ui_state, _session);
        {
            ...
            manager
                .append_message(pir_agent::messages::AgentMessage::ToolResult(
    ...
    async fn init_assembles_tree_and_starts_terminal() {
        ...
        // Terminal title set.
        assert!(terminal.title().contains("pir - "));
        // init is idempotent.
        mode.init().await;
        assert!(mode.is_initialized);
    }
    ...
    async fn tree_select_user_message_restores_leaf_and_editor_text() {
        ...
        let first_user_parent = entries
            ...
            .find_map(|e| match e.known() {
                Some(pir_agent::session::SessionEntry::Message(m))
    ...
    async fn tree_select_custom_message_restores_leaf_and_editor_text() {
        ...
        let ui = &mode.ui_state;
        {
            ...
            manager
                .append_custom_message_entry(
                    "test.type",
                    pir_ai::types::UserContent::Text("custom payload".to_string()),
                    true,
                    None,
                )
        ...
        let custom_parent = entries
            .iter()
            .find_map(|e| match e.known() {
                Some(pir_agent::session::SessionEntry::CustomMessage(_)) => {
                    Some(e.parent_id().map(str::to_owned))
                }
                _ => None,
            })
