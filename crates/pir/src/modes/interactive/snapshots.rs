...
//! against a golden file in `crates/pir/tests/snapshots/<name>.snap` (same
//! pattern as pir-tui `tests/snapshots.rs`). The snapshots are observations
//! of the frozen upstream behavior; regenerate with:
//!
//! ```sh
//! PIR_UPDATE_SNAPSHOTS=1 cargo test -p pir --lib -- snapshots
//! ```
//!
