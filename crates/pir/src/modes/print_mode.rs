...
//! pir has no detached-child registry (D-011); the handler exits directly
//! with the same codes — abort tokens do not outlive process death.

use std::io::Write;

use pir_agent::messages::AgentMessage;
use pir_ai::types::{ImageContent, StopReason};
...
fn session_header_json(session_manager: &SessionManager) -> Option<String> {
    ...
    serde_json::to_string(&pir_agent::session::FileEntry::Session(header.clone())).ok()
}
        ...
        if let Some(AgentMessage::Assistant(assistant)) = messages.last() {
                ...
                || assistant.stop_reason == StopReason::Aborted
            {
                ...
                for content in &assistant.content {
                    if let pir_ai::types::AssistantContent::Text(text) = content {
