...
impl RpcUiBridge {
    ...
    async fn dialog(&self, method_fields: Value, timeout_ms: Option<u64>) -> Option<Value> {
        let id = pir_ai::utils::uuid::random_uuid();
...
impl UiBridge for RpcUiBridge {
    ...
    fn notify(&self, message: &str, kind: NotifyType) {
        self.emit(json!({
            ...
            "id": pir_ai::utils::uuid::random_uuid(),
            "method": "notify",
            "message": message,
    ...
    fn set_widget(
        ...
        self.emit(json!({
            ...
            "widgetPlacement": options.and_then(|o| o.placement).map(|p| match p {
                pir_ext_host::api::WidgetPlacement::AboveEditor => "aboveEditor",
                pir_ext_host::api::WidgetPlacement::BelowEditor => "belowEditor",
            }),
        }));
    }
