...
/// `PI_SHARE_VIEWER_URL` → `PIR_SHARE_VIEWER_URL` (requirements §3.3) — the
...
/// `PIR_SHARE_VIEWER_URL` override (empty falls back to the default, like the
...
// - pir is always a native binary: upstream's `isBunBinary` detection
...
// - `PACKAGE_NAME` is the npm/distribution package name pir would be
...
pub const PACKAGE_NAME: &str = "pir";
...
/// joins `__dirname` and `process.execPath`; pir has only the executable
...
// first, then settings, then the built-in default (`https://pi.dev`). Any
...
// `modelCatalogUrl` settings keys are pir-specific (ADR-0002 §8); upstream
...
/// Pir-specific (ADR-0002 §8): override for the version-check endpoint
/// (default `LATEST_VERSION_URL` in [`crate::core::version_check`]).
pub const ENV_VERSION_CHECK_URL: &str = "PIR_VERSION_CHECK_URL";
/// Pir-specific (ADR-0002 §8): override for the install-telemetry endpoint
/// (default `DEFAULT_REPORT_INSTALL_URL` in [`crate::core::telemetry`]).
pub const ENV_TELEMETRY_URL: &str = "PIR_TELEMETRY_URL";
/// Pir-specific (ADR-0002 §8): override for the remote model catalog base
/// URL (default `DEFAULT_CATALOG_BASE_URL` in
/// [`crate::core::remote_catalog_provider`]).
pub const ENV_MODEL_CATALOG_URL: &str = "PIR_MODEL_CATALOG_URL";
...
mod tests {
    ...
    fn test_resolve_endpoint_defaults_without_overrides() {
        assert_eq!(
            resolve_endpoint(None, None, "https://pi.dev/api"),
            Some("https://pi.dev/api".to_string())
        );
        // Empty strings fall through to the next level (JS `||` semantics).
        assert_eq!(
            resolve_endpoint(Some(""), Some(""), "https://pi.dev/api"),
            Some("https://pi.dev/api".to_string())
        );
    }
...
mod self_update_tests {
    ...
    fn detect_install_method_from_paths() {
        assert_eq!(
            detect_install_method_from(
                "/home/u/.local/share/pnpm/global/5/.pnpm/pir@1.0.0/node_modules/pir/bin"
            ),
            InstallMethod::Pnpm
        );
        assert_eq!(
            detect_install_method_from("/home/u/.yarn/global/node_modules/.bin/pir"),
            InstallMethod::Yarn
        );
        assert_eq!(
            detect_install_method_from("/home/u/.bun/install/global/node_modules/pir/bin/pir"),
            InstallMethod::Bun
        );
        assert_eq!(
            detect_install_method_from("/usr/lib/node_modules/pir/bin/pir"),
            InstallMethod::Npm
        );
        assert_eq!(
            detect_install_method_from("/usr/local/bin/pir"),
            InstallMethod::Binary
        );
        // Case-insensitive, Windows separators tolerated.
        assert_eq!(
            detect_install_method_from("C:\\Users\\u\\AppData\\Roaming\\NPM\\pir.exe"),
            InstallMethod::Npm
        );
    }
    ...
    fn renamed_package_prepends_uninstall_step() {
        let command = get_self_update_command_for_method(
            InstallMethod::Npm,
            "pir",
            &target("pir-next", "pir-next@1.2.3"),
            None,
            &no_probe,
        )
        .expect("command");
        assert_eq!(command.steps.len(), 2);
        assert_eq!(command.steps[0].args, ["uninstall", "-g", "pir"]);
    ...
    fn pnpm_yarn_bun_command_shapes() {
        ...
        assert_eq!(
            yarn.step.args,
            ["global", "add", "--ignore-scripts", "pir@1.2.3"]
        );
    ...
    fn pnpm_global_root_extraction() {
        let root = pnpm_global_root_from_package_dir(Path::new(
            "/home/u/.local/share/pnpm/global/5/.pnpm/pir@1.0.0/node_modules/pir",
        ));
        assert_eq!(
            root,
