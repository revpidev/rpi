...
//! (2efa728) — the `pir config` resource-configuration TUI (T14-W3).
//!
...
use pir_tui::tui::{shared_component_from_boxed, Component, Focusable, Tui};
...
pub const CONFIG_USAGE: &str = "pir config [-l] [--approve|--no-approve]";
...
/// `TestTerminal`; no real `~/.pir`, process, or network is touched).
...
async fn select_config(
    ...
    let driver = std::thread::Builder::new()
        .name("pir-config-driver".to_string())
        .spawn(move || {
...
mod tests {
    ...
    fn test_local_requires_project_trust() {
        // A pre-existing `.pir/settings.json` requires trust; the headless
