...
//!   TTY-only. The release note renders through the pir-tui Markdown
...
/// literal `pir` mirrors `APP_NAME` (config.rs); the
...
pub struct ParsedUpdate {
    ...
    /// `--force` — reinstall pir even when the current version is latest
    ...
    /// Bare `pir update` prints the extensions-skipped note
...
/// executable location line uses the pir name (ADR-0001); upstream's
...
/// runner and release transport (tests inject fakes so no real `~/.pir`,
...
pub const INSTALL_USAGE: &str = "pir install <source> [-l] [--approve|--no-approve]";
/// `getPackageCommandUsage("remove")` (package-manager-cli.ts:83-84).
pub const REMOVE_USAGE: &str = "pir remove <source> [-l] [--approve|--no-approve]";
/// `getPackageCommandUsage("list")` (package-manager-cli.ts:87-88).
pub const LIST_USAGE: &str = "pir list [--approve|--no-approve]";
...
/// temporary cwd/agent dir here so no real `~/.pir` is touched).
...
mod tests {
    ...
    fn usage_lines_start_with_app_name() {
        // T14 review N-1: the usage strings carry the literal `pir`; bind
...
mod package_command_tests {
    ...
    fn test_help_texts_carry_usage_and_examples() {
        ...
        assert!(install.contains("pir install npm:@foo/bar"));
        ...
        assert!(remove.contains("pir uninstall <source> [-l]"));
        ...
        let packages = vec![
            ConfiguredPackage {
                ...
                installed_path: Some(PathBuf::from(
                    "/home/u/.pir/agent/npm/node_modules/left-pad",
                )),
            },
            ConfiguredPackage {
    ...
    fn test_install_local_initializes_fresh_project_settings() {
        ...
        // `-l` creates `.pir/settings.json` (package-command-paths.test.ts
        ...
        let settings: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(dirs.cwd.join(".pir/settings.json")).unwrap(),
        )
        .unwrap();
    ...
    fn test_install_local_blocked_when_project_untrusted() {
        // A pre-existing `.pir/settings.json` requires trust; without
        // `-a` the headless default (ask → false) blocks the write.
        let dirs = TestDirs::new();
        std::fs::create_dir_all(dirs.cwd.join(".pir")).unwrap();
        std::fs::write(dirs.cwd.join(".pir/settings.json"), "{}").unwrap();
        std::fs::create_dir_all(dirs.cwd.join("pkg/extensions")).unwrap();
        std::fs::write(dirs.cwd.join("pkg/extensions/index.ts"), "").unwrap();
    ...
    fn test_list_with_no_approve_ignores_project_packages() {
        ...
        std::fs::create_dir_all(dirs.cwd.join(".pir")).unwrap();
        std::fs::write(
            dirs.cwd.join(".pir/settings.json"),
...
mod update_cli_tests {
    ...
    async fn update_self_endpoint_failures_exit_1() {
        let dirs = TestDirs::new();
        // Non-OK response → "Could not determine latest pir version."
        let code = run_update_in(
            &args(&["update", "--self"]),
            &dirs.cwd,
            &dirs.agent_dir,
