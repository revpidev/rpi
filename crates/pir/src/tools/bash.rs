...
fn get_shell_env() -> HashMap<String, String> {
    ...
    if let Some(home) = std::env::var_os("HOME") {
        let bin = PathBuf::from(&home).join(".pir").join("bin");
        ...
        if self.expose_session_environment {
            if let Some(ref s) = self.session_env {
                ...
                if let Some(ref p) = s.provider {
                    env.insert("PIR_PROVIDER".into(), p.clone());
                }
                if let Some(ref m) = s.model {
                    env.insert("PIR_MODEL".into(), m.clone());
                }
                if let Some(ref l) = s.reasoning_level {
                    env.insert("PIR_REASONING_LEVEL".into(), l.clone());
                }
            }
        }
