...
mod bash_tool_tests {
            ...
            BashToolOptions {
                spawn_hook: Some(Arc::new(move |ctx: BashSpawnContext| {
                    *hook_called2.lock().unwrap() = true;
                    ...
                    let mut env = ctx.env;
                    env.insert("RPI_TEST_HOOK".into(), "yes".into());
    ...
    async fn test_rpi_session_env_injected() {
        ...
        assert!(
            out.contains("RPI_SESSION_ID=test-session-123"),
            "out: {out}"
        );
        assert!(
            out.contains("RPI_SESSION_FILE=/tmp/test.jsonl"),
            "out: {out}"
        );
        assert!(out.contains("RPI_PROVIDER=test-provider"), "out: {out}");
        assert!(out.contains("RPI_MODEL=test-model"), "out: {out}");
        assert!(out.contains("RPI_REASONING_LEVEL=high"), "out: {out}");
    }
    ...
    async fn test_session_env_resolved_per_command_start() {
        ...
        // and the session values are re-injected over any inherited `RPI_*`.
        let ctx = ToolContext {
            cwd: PathBuf::from("."),
            session_env: Some(std::sync::Arc::new(std::sync::RwLock::new(SessionEnv {
        ...
        assert!(
            ...
            "session env re-injected per command, inherited RPI_MODEL stripped: {out2}"
        );

        // A model switch takes effect immediately for the next command: the
        // tool rebuilt with the fresh SessionEnv resolves RPI_MODEL anew.
        ...
        let out3 = run_bash(&switched, "echo $RPI_MODEL", None).await.unwrap();
        assert!(
