...
impl TempDir {
    fn new() -> Self {
        ...
        let dir =
            std::env::temp_dir().join(format!("rpi-rpc-test-{}-{nanos}-{id}", std::process::id()));
...
fn signal_exit_code(signal: libc::c_int, expected_code: i32) {
    ...
    let mut child = std::process::Command::new(env!("CARGO_BIN_EXE_rpi-rpc"))
        .args([
            "--no-session",
            "--provider",
            "faux",
...
fn rpi_rpc_bin_end_to_end() {
  ...
  "providers": {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_rpi-rpc"))
        .args([
            "--no-session",
            "--provider",
            "faux",
