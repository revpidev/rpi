...
impl TempDir {
    fn new() -> Self {
        ...
        let dir = std::env::temp_dir().join(format!(
            "rpi-agent-session-test-{}-{nanos}-{id}",
            std::process::id()
        ));
...
async fn thinking_level_reaches_provider_stream_and_lands_in_transcript() {
    ...
    // rpi's pinned `StreamFn` shape (design §4.4) carries plain
    ...
    assert_eq!(
        fixture
            ...
            .copied()
            .flatten(),
        Some(rpi_ai::types::ThinkingLevel::High)
    );
    ...
    assert!(last_assistant.content.iter().any(|block| matches!(
        block,
        rpi_ai::types::AssistantContent::Thinking(thinking) if thinking.thinking == "hmm"
    )));
}
