...
//! vs the normalized diff of rpi's corresponding implementation loading the
//! same input.
//!
//! Normalization conventions (reusing `rpi_test_support`; no second
//! implementation):
...
//! - `.pi` → `.rpi` is the intentional rename (requirements §1.4 /
//!   ADR-0001): the e2e goldens already use the `.rpi` spelling, and the
//!   tests run both sides against one directory tree containing both `.rpi`
//!   and `.pi` copies.
...
use rpi::core::keybindings::migrate_keybindings_config;
use rpi::core::prompt_templates::{parse_command_args, substitute_args};
use rpi::core::resource_loader::{DefaultResourceLoader, DefaultResourceLoaderOptions};
use rpi::core::settings_manager::{
...
impl TestDir {
    fn new(name: &str) -> Self {
        ...
        let dir = std::env::temp_dir().join(format!(
            "rpi-parity-resources-{name}-{}-{nanos}-{id}",
            std::process::id()
        ));
...
/// tree, duplicate every `.rpi` dir as `.pi` (upstream reads `.pi`, rpi
/// reads `.rpi`), create the untrackable `repo/.git` marker.
