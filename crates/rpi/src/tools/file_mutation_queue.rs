...
mod tests {
    ...
    async fn test_serializes_same_file() {
        ...
        let path = std::env::temp_dir().join("rpi-mq-same-file-test");
        let log1 = log.clone();
    ...
    async fn test_parallel_different_files() {
        ...
        let path_a = std::env::temp_dir().join("rpi-mq-parallel-a");
        let path_b = std::env::temp_dir().join("rpi-mq-parallel-b");
    ...
    async fn test_return_value_propagated() {
        let path = std::env::temp_dir().join("rpi-mq-return-test");
    ...
    async fn test_gc_removes_entry() {
        let path = std::env::temp_dir().join("rpi-mq-gc-test");
