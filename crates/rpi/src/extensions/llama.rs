...
//! lives in `crates/rpi/src/modes/interactive/components/llama_view.rs`).
//!
...
pub fn inline_extension() -> rpi_ext_host::loader::InlineExtension {
...
async fn llama_command(ctx: rpi_ext_host::api::ExtensionCommandContext) -> Result<(), String> {
    let is_tui = matches!(ctx.mode(), Ok(rpi_ext_host::types::ExtensionMode::Tui));
    let ui = ctx.ui().map_err(|e| e.to_string())?;
    if !is_tui {
        ui.notify(
            "The /llama manager requires the interactive TUI",
            rpi_ext_host::api::NotifyType::Error,
        );
        return Ok(());
    }
    let bridge = ui
...
use rpi_ai::auth::{AuthResult, ModelsError};
