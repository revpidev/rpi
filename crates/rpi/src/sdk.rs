...
//! (2efa728).
//!
//! The `setDefaultStreamFn(streamSimple)` fallback (sdk.ts:36) has no rpi
...
pub struct CreateAgentSessionOptions {
    ...
    /// Global config directory. Default: `~/.rpi/agent`.
    ...
    /// no-op runner.
    pub extension_host: Option<Arc<rpi_ext_host::host::NativeExtensionHost>>,
}
...
pub async fn create_agent_session(
    ...
    let thinking_level = match &model {
        ...
        Some(model) => {
            rpi_ai::models::clamp_thinking_level(model, thinking_level.expect("set above"))
        }
    };
    ...
    let convert_to_llm = {
        ...
        Arc::new(move |messages: Vec<AgentMessage>| {
            ...
            Box::pin(async move {
                let converted = rpi_agent::convert_to_llm(&messages);
                let block_images = {
                ...
                converted
                    ...
                    .map(|message| match message {
                        Message::User(mut user) => {
                            if let rpi_ai::types::UserContent::Blocks(blocks) = &mut user.content {
    ...
    let stream_fn = {
        ...
        Arc::new(
            move |model: Model, context: rpi_ai::types::Context, options: StreamOptions| {
                ...
                // thinking mapping; the rpi `StreamFn` shape (design §4.4)
...
fn filter_user_image_blocks(blocks: &mut Vec<rpi_ai::types::UserContentBlock>) {
...
fn filter_tool_result_image_blocks(blocks: &mut Vec<rpi_ai::types::ToolResultContent>) {
...
) -> Option<rpi_ai::types::ThinkingBudgets> {
    budgets.map(|budgets| rpi_ai::types::ThinkingBudgets {
