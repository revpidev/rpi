...
//! - `.pi` → `.rpi`, `PI_` → `RPI_` (ADR-0001). The upstream discovery mode
//!   `"pi"` is [`SkillDiscoveryMode::Rpi`] here.
...
/// `Rpi` (ADR-0001 rename): loose `.md` files at the scan root count as
...
/// - otherwise, in [`SkillDiscoveryMode::Rpi`] mode, loose `.md` files at
...
pub fn load_skills_from_dir(dir: &Path, source: SkillLoadSource) -> LoadSkillsResult {
    ...
    for path in collect_skill_entries(dir, SkillDiscoveryMode::Rpi) {
...
pub struct DiscoverSkillsOptions {
    ...
    pub cwd: PathBuf,
    /// Agent config directory (`~/.rpi/agent` by default).
    pub agent_dir: PathBuf,
    /// User home directory; `~/.agents/skills` is skipped when `None`.
    pub home_dir: Option<PathBuf>,
    /// Trust gate for project-local resources (`.rpi/skills` and ancestor
...
/// scanned in [`SkillDiscoveryMode::Rpi`]); pattern entries only filter
...
pub fn resolve_local_skill_entries(
    ...
    for entry in &plain {
        ...
        if resolved.is_file() {
            ...
            all_files.extend(collect_skill_entries(&resolved, SkillDiscoveryMode::Rpi));
        }
    }
...
pub fn discover_skill_paths(options: &DiscoverSkillsOptions) -> Vec<ResolvedSkillPath> {
    ...
    // User skills from `{agentDir}/skills` (`~/.rpi/agent/skills`).
    ...
    for path in collect_skill_entries(&agent_dir.join("skills"), SkillDiscoveryMode::Rpi) {
        let enabled =
...
pub struct LoadSkillsOptions {
    ...
    /// Also load `{agentDir}/skills` and `{cwd}/.rpi/skills` first
...
mod tests {
    ...
    fn test_collect_skill_entries_pir_mode_loose_md_and_skill_roots() {
        ...
        let entries = collect_skill_entries(&root, SkillDiscoveryMode::Rpi);
    ...
    fn test_load_skills_path_warnings_and_source_classification() {
        ...
        let project_skill = cwd.join(".rpi/skills/p/SKILL.md");
    ...
    fn test_discover_skill_paths_rank_order_and_trust_gate() {
        ...
        write(
            &options.cwd.join(".rpi/skills/proj-auto/SKILL.md"),
            "---\ndescription: x\n---\n",
        );
        write(
        ...
        let project_settings_dir = options.cwd.join(".rpi/declared");
        write(
