    ...
    for (name, value) in extension_flag_values {
        match registered_flags.get(name) {
            ...
            Some(flag) => match (flag.flag_type, value) {
                (rpi_ext_host::types::FlagType::Boolean, _) => {
                    host.runtime()
                        .set_flag_value(name, rpi_ext_host::types::FlagValue::Boolean(true));
                }
                (rpi_ext_host::types::FlagType::String, UnknownFlagValue::String(value)) => {
                    host.runtime().set_flag_value(
                        name,
                        rpi_ext_host::types::FlagValue::String(value.clone()),
                    );
                }
                (rpi_ext_host::types::FlagType::String, UnknownFlagValue::Boolean(_)) => {
