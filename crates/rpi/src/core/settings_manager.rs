...
impl FileSettingsStorage {
    ...
    /// `{cwd}/.rpi/settings.json` (settings-manager.ts:196).
...
impl SettingsManager {
    ...
    /// `versionCheckUrl` setting — rpi-specific (ADR-0002 §8, no upstream
    ...
    /// `telemetryUrl` setting — rpi-specific (ADR-0002 §8): overrides the
    ...
    /// `modelCatalogUrl` setting — rpi-specific (ADR-0002 §8): overrides the
    ...
    /// then the `RPI_CLEAR_ON_SHRINK === "1"` env fallback, default false.
    ...
    /// then the `RPI_HARDWARE_CURSOR === "1"` env fallback.
...
mod tests {
    ...
    /// Upstream `beforeEach`: fresh agent dir and `project/.rpi`.
    fn test_dirs() -> TestDirs {
        let tmp = TempDir::new();
        let agent_dir = tmp.path().join("agent");
        let project_dir = tmp.path().join("project");
        std::fs::create_dir_all(&agent_dir).unwrap();
        std::fs::create_dir_all(project_dir.join(".rpi")).unwrap();
        TestDirs {
            _tmp: tmp,
            agent_dir,
            project_dir,
    ...
    fn project_path(dirs: &TestDirs) -> PathBuf {
        dirs.project_dir.join(".rpi").join("settings.json")
    }
    ...
    fn test_does_not_create_rpi_dir_when_only_reading() {
        ...
        std::fs::remove_dir_all(dirs.project_dir.join(".rpi")).unwrap();
    ...
    /// The rpi-specific endpoint URL settings (ADR-0002 §8): absent by
