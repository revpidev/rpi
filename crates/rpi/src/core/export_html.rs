...
//!   (`getExportTemplateDir`, config.ts:412-419); rpi embeds them at compile
//!   time (single-file binary, ADR-0002 §8). There is no
//!   `RPI_PACKAGE_DIR`-style override for the template.
...
//!   ANSI→HTML pipeline (`tool-renderer.ts` / `ansi-to-html.ts`), and rpi has
...
//!   in rpi): `None` resolves straight to the terminal-detected default.
