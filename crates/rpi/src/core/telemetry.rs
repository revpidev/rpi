...
//! - `PI_TELEMETRY` → `RPI_TELEMETRY` (ADR-0001); the env layer itself lives
//!   in [`crate::core::environment::telemetry_enabled_override`].
//! - T14-W6a (ADR-0002 §8): the ping endpoint is configurable via the
//!   `RPI_TELEMETRY_URL` env var or the `telemetryUrl` setting
//!   ([`report_install_endpoint`], rpi-specific — upstream hardcodes
...
pub const DEFAULT_REPORT_INSTALL_URL: &str = "https://revpi.dev/api/report-install";

/// `AbortSignal.timeout(5000)` (interactive-mode.ts:1031).
pub const REPORT_INSTALL_TIMEOUT: Duration = Duration::from_millis(5_000);

/// `isInstallTelemetryEnabled` (telemetry.ts:8-12): a set `RPI_TELEMETRY`
...
/// `RPI_TELEMETRY_URL` env > `telemetryUrl` setting >
/// [`DEFAULT_REPORT_INSTALL_URL`]; the literal `off` disables the ping
/// (`None` — no request is made). Rpi-specific; upstream hardcodes the URL.
...
/// fire-and-forget GET to `{endpoint}?version={version}` with the rpi user
...
pub async fn report_install(
    ...
    let _ = transport
        .get(&url, &rpi_user_agent(version), REPORT_INSTALL_TIMEOUT)
        .await;
}
...
mod tests {
    ...
    //! with a scripted transport instead of fetch stubs. No `RPI_*` variable
    ...
    fn install_telemetry_enabled_composition() {
        ...
        // Empty RPI_TELEMETRY maps to Some(false) upstream (set-but-empty
        // disables) — covered by the environment.rs override tests.
    }

    #[test]
    fn report_install_endpoint_defaults_and_settings_override() {
        // Read-only env use: no test writes RPI_TELEMETRY_URL.
        assert_eq!(crate::config::ENV_TELEMETRY_URL, "RPI_TELEMETRY_URL");
        assert_eq!(
    ...
    async fn report_install_pings_version_with_user_agent() {
        ...
        assert_eq!(calls[0].1, rpi_user_agent("1.2.3"));
