...
impl CommandRequest {
    ...
    /// upstream echoes the raw argv, but rpi's red line forbids credentials
...
/// output (package + top-level + auto-discovered), backing `rpi config`.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
...
impl DefaultPackageManager {
    ...
    /// `-e` temporary scope (`~/.rpi/agent/tmp/extensions`).
    ...
    /// resource loader; this full form backs `rpi config`.
    ...
    fn add_auto_discovered_resources(
        ...
        if project_trusted {
            // Project extensions / skills from `.rpi/`.
            ...
            self.add_auto_resources(
                ResourceType::Skills,
                collect_skill_entries(&project_base_dir.join("skills"), SkillDiscoveryMode::Rpi),
                &project_metadata,
        ...
        self.add_auto_resources(
            ResourceType::Skills,
            collect_skill_entries(&global_base_dir.join("skills"), SkillDiscoveryMode::Rpi),
            &user_metadata,
...
/// wasm extension packages — `rpi-extension.json` with a `wasm` field, or
...
fn collect_resource_files(dir: &Path, resource_type: ResourceType) -> Vec<PathBuf> {
    match resource_type {
        ResourceType::Skills => collect_skill_entries(dir, SkillDiscoveryMode::Rpi),
...
mod update_tests {
    ...
    fn test_resolve_all_includes_auto_discovered_with_enabled_flags() {
        ...
        write_skill(&dirs.cwd.join(".rpi/skills"), "bar");
