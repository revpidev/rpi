...
pub struct BeforeAgentStartMessage {
    ...
    pub content: Option<rpi_ai::types::UserContent>,
    pub display: bool,
...
pub struct SessionBeforeCompactResult {
    ...
    pub compaction: Option<rpi_agent::compaction::CompactionResult>,
}
...
/// `rpi_ext_host::runner::ExtensionRunnerCore::emit_tool_call`). Applied
        ...
        move |context: rpi_agent::agent_loop::BeforeToolCallContext, _signal| {
            ...
            Box::pin(async move {
                ...
                as futures::future::BoxFuture<
                    'static,
                    Option<rpi_agent::agent_loop::BeforeToolCallResult>,
                >
        },
    )
}
...
pub fn extension_on_response_callback(
    runner_ref: ExtensionRunnerRef,
) -> rpi_ai::types::OnResponseCallback {
    Arc::new(move |response: rpi_ai::types::ProviderResponse, _model| {
        let runner = read_runner(&runner_ref);
        Box::pin(async move {
