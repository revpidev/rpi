...
//!   :425-451).
//! - ADR-0001 renames apply: `.pi` → `.rpi`, `PI_` → `RPI_`.
//!
...
struct FileResourceDiscovery<'a> {
    cwd: &'a Path,
    agent_dir: &'a Path,
    /// Trust gate for project-local auto discovery (`.rpi/prompts`,
    /// `.rpi/themes`). Project settings entries arrive empty from an
...
mod tests {
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let dir = std::env::temp_dir().join(format!(
                "rpi-resource-loader-test-{}-{nanos}-{id}",
                std::process::id()
            ));
    ...
    fn test_dedupe_prompts_first_wins_with_collision_diagnostic() {
        ...
        let second = prompt("commit", Path::new("/project/.rpi/prompts/commit.md"));
        ...
        assert_eq!(
            ...
            Some(Path::new("/project/.rpi/prompts/commit.md"))
        );
        ...
        assert_eq!(
            ...
            PathBuf::from("/project/.rpi/prompts/commit.md")
        );
    }
    ...
    fn test_discover_file_resource_paths_rank_sort_and_trust_gate() {
        ...
        // project auto (rank 1)
        write(
            &cwd.join(".rpi").join("prompts").join("auto-project.md"),
            "project",
        );
