    ...
    fn render(&self, width: usize) -> Vec<String> {
        ...
        for entry in &entries {
            match entry.known() {
                ...
                // `entry.type === "usage"` → session totals (footer.ts:89-92,
                // #9668).
        ...
        // `experimental_enabled` (core/environment.rs, RPI_EXPERIMENTAL=1).
...
mod tests {
    ...
    fn format_cwd_replaces_home_prefix() {
        ...
        assert_eq!(
            format_cwd_for_footer(Path::new("/home/user/projects/rpi"), Some(home)),
            "~/projects/rpi"
        );
        // Outside home: unchanged.
        assert_eq!(
            format_cwd_for_footer(Path::new("/opt/rpi"), Some(home)),
            "/opt/rpi"
        );
    ...
    async fn truncation_falls_back_in_stages() {
        ...
        assert!(
            rpi_tui::utils::visible_width(&stats) <= 30,
            "width: {stats:?}"
        );
