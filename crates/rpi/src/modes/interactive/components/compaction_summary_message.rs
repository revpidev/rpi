//! Compaction summary message — port of
//! `packages/coding-agent/src/modes/interactive/components/compaction-summary-message.ts`
//! @ pi 0.82.1 (2efa728).
//!
//! Intentional differences:
//! - The theme is passed explicitly (`Arc<Theme>`) instead of read from the
//!   global `theme` getter (theme.ts:799-816); `markdownTheme` is explicit.
//! - Upstream `tokensBefore.toLocaleString()` is locale-dependent; the port
//!   pins en-US `,` grouping ([`super::util::to_locale_string`]).
//! - Upstream extends `Box`; the port composes a `Box` field (identical
//!   render contract).

use std::boxed::Box as StdBox;
use std::sync::Arc;

use rpi_agent::messages::CompactionSummaryMessage;
use rpi_tui::components::markdown::{DefaultTextStyle, Markdown, MarkdownTheme};
use rpi_tui::components::r#box::Box as TuiBox;
use rpi_tui::components::spacer::Spacer;
use rpi_tui::components::text::Text;
use rpi_tui::tui::Component;

use crate::core::themes::Theme;

use super::keybinding_hints::key_text;
use super::util::to_locale_string;

/// Component that renders a compaction message with collapsed/expanded state
/// (compaction-summary-message.ts:10-59). Uses the same background color as
/// custom messages for visual consistency.
pub struct CompactionSummaryMessageComponent {
    expanded: bool,
    message: CompactionSummaryMessage,
    markdown_theme: Arc<MarkdownTheme>,
    theme: Arc<Theme>,
    content_box: TuiBox,
}

impl CompactionSummaryMessageComponent {
    pub fn new(
        message: CompactionSummaryMessage,
        theme: Arc<Theme>,
        markdown_theme: Arc<MarkdownTheme>,
    ) -> Self {
        let mut component = Self {
            expanded: false,
            message,
            markdown_theme,
            theme,
            content_box: TuiBox::new(0, 0, None),
        };
        component.update_display();
        component
    }

    /// `setExpanded` (compaction-summary-message.ts:22-25).
    pub fn set_expanded(&mut self, expanded: bool) {
        self.expanded = expanded;
        self.update_display();
    }

    /// `updateDisplay` (compaction-summary-message.ts:32-57).
    fn update_display(&mut self) {
        let bg = {
            let theme = Arc::clone(&self.theme);
            Box::new(move |t: &str| theme.bg("customMessageBg", t))
        };
        let mut content_box = TuiBox::new(1, 1, Some(bg));

        let token_str = to_locale_string(self.message.tokens_before);
        let label = self
            .theme
            .fg("customMessageLabel", "\u{1b}[1m[compaction]\u{1b}[22m");
        content_box.add_child(StdBox::new(Text::new(label, 0, 0, None)));
        content_box.add_child(StdBox::new(Spacer::new(1)));

        if self.expanded {
            let header = format!("**Compacted from {token_str} tokens**\n\n");
            let color = {
                let theme = Arc::clone(&self.theme);
                Box::new(move |t: &str| theme.fg("customMessageText", t))
            };
            content_box.add_child(StdBox::new(Markdown::new(
                format!("{header}{}", self.message.summary),
                0,
                0,
                Arc::clone(&self.markdown_theme),
                Some(DefaultTextStyle {
                    color: Some(color),
                    ..Default::default()
                }),
                None,
            )));
        } else {
            let text = format!(
                "{}{}{}",
                self.theme.fg(
                    "customMessageText",
                    &format!("Compacted from {token_str} tokens (")
                ),
                self.theme.fg("dim", &key_text("app.tools.expand")),
                self.theme.fg("customMessageText", " to expand)"),
            );
            content_box.add_child(StdBox::new(Text::new(text, 0, 0, None)));
        }

        self.content_box = content_box;
    }
}

impl Component for CompactionSummaryMessageComponent {
    fn render(&self, width: usize) -> Vec<String> {
        self.content_box.render(width)
    }

    fn invalidate(&mut self) {
        self.content_box.invalidate();
        self.update_display();
    }

    fn set_expanded(&mut self, expanded: bool) {
        // `setToolsExpanded` chat walk (upstream `isExpandable` duck-typing);
        // inherent methods win on concrete receivers, so no recursion.
        self.set_expanded(expanded);
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::core::themes::load_theme;
    use crate::modes::interactive::theme::markdown_theme;
    use rpi_agent::messages::CompactionSummaryRole;

    fn theme() -> Arc<Theme> {
        Arc::new(load_theme("dark", None).expect("builtin dark theme"))
    }

    fn message() -> CompactionSummaryMessage {
        CompactionSummaryMessage {
            role: CompactionSummaryRole::CompactionSummary,
            summary: "The user asked about X.".into(),
            tokens_before: 12345,
            timestamp: 0,
        }
    }

    fn strip_ansi(input: &str) -> String {
        let mut out = String::with_capacity(input.len());
        let mut chars = input.chars().peekable();
        while let Some(c) = chars.next() {
            if c == '\u{1b}' && chars.peek() == Some(&'[') {
                chars.next();
                for c in chars.by_ref() {
                    if c == 'm' {
                        break;
                    }
                }
            } else {
                out.push(c);
            }
        }
        out
    }

    #[test]
    fn collapsed_shows_label_and_hint() {
        let component = CompactionSummaryMessageComponent::new(
            message(),
            theme(),
            markdown_theme(&load_theme("dark", None).unwrap()),
        );
        let lines = component.render(40);
        let stripped = strip_ansi(&lines.join("\n"));
        assert!(stripped.contains("[compaction]"));
        assert!(stripped.contains("Compacted from 12,345 tokens ("));
        assert!(stripped.contains("ctrl+o")); // keyText("app.tools.expand")
        assert!(stripped.contains(" to expand)"));
        assert!(!stripped.contains("The user asked"));
    }

    #[test]
    fn expanded_shows_summary_markdown() {
        let mut component = CompactionSummaryMessageComponent::new(
            message(),
            theme(),
            markdown_theme(&load_theme("dark", None).unwrap()),
        );
        component.set_expanded(true);
        let stripped = strip_ansi(&component.render(40).join("\n"));
        assert!(stripped.contains("Compacted from 12,345 tokens"));
        assert!(stripped.contains("The user asked about X."));
        assert!(!stripped.contains("ctrl+o"));
    }
}
