...
//! - Project-scope settings writes are gated upstream by the `rpi config`
...
//! - `CONFIG_DIR_NAME` is `.rpi` (ADR-0001 rename of `.pi`).
...
impl ResourceList {
    ...
    fn set_project_top_level_override(
        ...
        .map_err(|error| {
            ...
            // channel here.
            tracing::error!("rpi config: failed to write project settings: {error}");
            error
        })
        .is_ok()
    }
    ...
    /// by the `rpi config` command, so the setter `Result` cannot fail here
    /// (upstream would throw).
    /// The `setProject*Paths` / `set*Paths` dispatch
    /// (config-selector.ts:557-577, 689-694). Project writes are trust-gated
    /// by the `rpi config` command, but can still fail (pre-existing
    /// unparseable `.rpi/settings.json`, storage IO errors) — the error is
...
mod tests {
    ...
    fn install_keybindings() {
        ...
        rpi_tui::keys::set_kitty_protocol_active(false);
    }
    ...
    fn sample_paths(dirs: &TestDirs) -> ScopedResolvedPaths {
        ...
        let project_skill = dirs.cwd.join(".rpi/skills/deploy/SKILL.md");
    ...
    /// Strip ANSI escape sequences (shared with rpi-tui tests).
    ...
    fn project_settings(dirs: &TestDirs) -> serde_json::Value {
        let content = std::fs::read_to_string(dirs.cwd.join(".rpi/settings.json")).unwrap();
        serde_json::from_str(&content).unwrap()
    }
