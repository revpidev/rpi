...
//! by `rpi-agent`'s `AgentTool::execute` parameters and are **not** part of
...
/// Upstream names are `PI_*`; Rpi intentionally renames the prefix to `RPI_*`
...
pub(crate) mod test_helpers {
    ...
    impl TempDir {
        pub fn new() -> Self {
            let dir = std::env::temp_dir().join(format!("rpi-test-{}", super::random_hex_16()));
