...
impl FauxProvider {
    ...
    /// The injected stream function (pir-agent `StreamFn` shape). Requires a
    ...
    for message in &context.messages {
        let text = match message {
            Message::User(u) => match &u.content {
                pir_ai::types::UserContent::Text(t) => t.clone(),
                pir_ai::types::UserContent::Blocks(blocks) => content_to_text(blocks),
            },
            Message::Assistant(a) => assistant_content_to_text(&a.content),
            Message::ToolResult(tr) => tool_result_to_text(tr),
        };
        let role = match message.role() {
            pir_ai::types::Role::User => "user",
            pir_ai::types::Role::Assistant => "assistant",
            pir_ai::types::Role::ToolResult => "toolResult",
        };
...
mod tests {
    ...
    fn user_context(text: &str) -> Context {
        Context {
            system_prompt: None,
            messages: vec![Message::User(pir_ai::types::UserMessage {
                role: pir_ai::types::UserRole::User,
                content: pir_ai::types::UserContent::Text(text.to_owned()),
                timestamp: 0,
            })],
            tools: None,
        }
    ...
    async fn collect(
        stream: pir_agent::stream_fn::BoxStream<'static, StreamEvent>,
    ) -> Vec<StreamEvent> {
