...
If the invocation includes an implementation request, first launch one async `worker` to implement the approved scope. If the current diff is already the target, start with review. The sequence can be launched up front with `steps` when it is already clear, or continued as follow-up single-agent runs after each async completion. For an initial run, leave `async` at its default so the main chat is unblocked; do not pass `async: false` unless I explicitly want a foreground blocking run. Use only one writer against the active worktree at a time unless I explicitly ask for isolated worktrees.
