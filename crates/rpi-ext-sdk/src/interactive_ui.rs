//! Interactive custom UI ABI (ADR-0024) — guest-side protocol types.
//!
//! v0.1.4 **C0** freezes the wire shape of the seven additive `ui.*`
//! host-calls shared by the wasm (L1) and native (L0) carriers: the method
...
///
/// The wasm carrier uses [`RpiHost`]; native plugins implement it over the
/// abi_stable `RpiHostCalls` trampoline (`rpi_ext_host::interactive_ui`).
pub trait HostCall {
...
/// The default wasm guest transport (`rpi_host_call`).
#[derive(Clone, Copy, Debug, Default)]
pub struct RpiHost;

impl HostCall for RpiHost {
    fn call(&self, method: &str, args: Value) -> Result<Value, InteractiveUiError> {
        crate::host_call_typed(method, args)
    }
}
...
///
/// The native carrier (`rpi_ext_host::interactive_ui::Component`) freezes
/// the identical method set (G11 item 7).
pub trait Component {
