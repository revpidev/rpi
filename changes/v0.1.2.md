...
本版本主线：第四个第一方插件 rpi-ext-statusline 落地（TE12）、一批交互与模型运行时缺陷修复（会话切换扩展重挂 / apiKey 配置 / model_select 事件 /onboarding 品牌文案）、`/changelog` 接入真实更新日志，以及 subagents skill 文档按结构化入口本地化（ADR-0021）。

## 第一方插件(新)

- **rpi-ext-statusline**：CC 兼容脚本式自定义 statusline（L0 原生插件）。`settings.json` 写 `statusLine` 键即启用（命令 + padding/裁剪参数），两档 placement；脚本按 CC statusline JSON 协议 stdin/stdout 驱动，零新增 ABI。实机追补：事件到达即必跑脚本（切换模型/思考档位/分支即时刷新）、新会话 transcript latch 竞态（mtime 下界拒收上一会话）、数据指纹轮询自愈（1s 轮询五项 ctx 事实，不依赖宿主事件投递）。
